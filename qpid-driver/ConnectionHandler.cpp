...
#include "qpid/driver/ConnectionHandler.h"
...
namespace qpid {
	namespace driver {
		...
		ConnectionHandler::ConnectionHandler(
			const amqp::ConnectionOptions& s, ProtocolVersion& v, Bounds& b)
			: StateManager(NOT_STARTED), amqp::ConnectionOptions(s),
		...
		{
			if (getState() == CLOSED) {
				throw sys::Exception("Received frame on closed connection");
			}
			...
			try {
				if (frame.getChannel() != 0 || !invoke(static_cast<ConnectionOperations&>(*this), *body)) {
					...
					case CLOSING:
						...
						break;
					default:
						throw sys::Exception("Cannot receive frames on non-zero channel until connection is established.");
					}
				}
			}
		...
		void ConnectionHandler::outgoing(AMQFrame& frame)
		{
			if (getState() == OPEN)
				...
				throw sys::TransportFailure(errorText.empty() ? "Connection is not open." : errorText);
		}
		...
		void ConnectionHandler::waitForOpen()
		{
			waitFor(ESTABLISHED);
			if (getState() == FAILED) {
				throw sys::TransportFailure(errorText);
			}
		...
		void ConnectionHandler::close()
		{
			...
			case OPEN:
				if (setState(CLOSING, OPEN)) {
					proxy.close(200, OK);
					if (amqp::ConnectionOptions::heartbeat) {
						//heartbeat timer is turned off at this stage, so don't wait indefinately
						if (!waitFor(FINISHED, qpid::sys::NanoDuration(amqp::ConnectionOptions::heartbeat * qpid::sys::TIME_SEC))) {
		...
		void ConnectionHandler::start(const FieldTable& /*serverProps*/, const Array& mechanisms, const Array& /*locales*/)
		{
			...
			sasl = sys::SaslFactory::getInstance().create(username,
				password,
				service,
				host,
				minSsf,
			...
			if (mechanism.empty()) {
				...
				std::vector<std::string> allowed = sys::split(mechanism, " ");
				...
				if (mechlist.empty()) {
					throw sys::Exception(QPID_MSG("Desired mechanism(s) not valid: " << mechanism << " (supported: " << join(supported) << ")"));
				}
			}
			...
			if (sasl.get()) {
				...
				if (haveAnonymous && (mechanism.empty() || mechanism.find(ANONYMOUS) != std::string::npos)) {
					...
					if (!mechanism.empty()) throw sys::Exception(QPID_MSG("Desired mechanism(s) not valid: " << mechanism << "; client supports PLAIN or ANONYMOUS, broker supports: " << join(mechlist)));
					throw sys::Exception(QPID_MSG("No valid mechanism; client supports PLAIN or ANONYMOUS, broker supports: " << join(mechlist)));
				}
			}
		}
			...
			uint16_t heartbeatMin, uint16_t heartbeatMax)
		{
			...
			uint16_t heartbeat = amqp::ConnectionOptions::heartbeat;
			heartbeat = heartbeat < heartbeatMin ? heartbeatMin :
				heartbeat > heartbeatMax ? heartbeatMax :
				heartbeat;
			amqp::ConnectionOptions::heartbeat = heartbeat;
		...
		void ConnectionHandler::openOk(const Array& knownBrokers)
		{
			...
			for (i = knownBrokers.begin(); i != knownBrokers.end(); ++i)
				knownBrokersUrls.push_back(sys::Url((*i)->get<std::string>()));
			if (sasl.get()) {
				securityLayer = sasl->getSecurityLayer(maxFrameSize);
				operUserId = sasl->getUserId();
			}
			setState(OPEN);
			QPID_LOG(debug, "Known-brokers for connection: " << sys::formatList(knownBrokersUrls));
		}
