...
struct IOThreadOptions : public qpid::sys::Options {
    int maxIOThreads;
...
public:
    HeartbeatTask(NanoDuration p, ConnectionImpl& t) :
...
boost::shared_ptr<ConnectionImpl> ConnectionImpl::create(framing::ProtocolVersion version, const amqp::ConnectionOptions& settings)
{
    boost::shared_ptr<ConnectionImpl> instance(new ConnectionImpl(version, settings), boost::bind(&ConnectionImpl::release, _1));
    return instance;
}

ConnectionImpl::ConnectionImpl(framing::ProtocolVersion v, const amqp::ConnectionOptions& settings)
...
void ConnectionImpl::open()
{
    ...
    try {
        ...
        connector.reset();
        throw sys::TransportFailure(e.what());
    }
    connector->init();

    // Enable heartbeat if requested
    uint16_t heartbeat = static_cast<amqp::ConnectionOptions&>(handler).heartbeat;
    if (heartbeat) {
...
const amqp::ConnectionOptions& ConnectionImpl::getNegotiatedSettings()
{
    return handler;
}

std::vector<qpid::sys::Url> ConnectionImpl::getInitialBrokers() {
