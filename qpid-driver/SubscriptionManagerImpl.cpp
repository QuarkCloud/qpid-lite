...
namespace qpid {
	namespace driver {
		...
		bool SubscriptionManagerImpl::get(Message& result, const std::string& queue, sys::NanoDuration timeout) {
			LocalQueue lq;
		...
		Message SubscriptionManagerImpl::get(const std::string& queue, sys::NanoDuration timeout) {
			Message result;
			if (!get(result, queue, timeout))
				throw sys::Exception("Timed out waiting for a message");
			return result;
		}
		...
		Subscription SubscriptionManagerImpl::getSubscription(const std::string& name) const {
			...
			if (i == subscriptions.end())
				throw sys::Exception(QPID_MSG("Subscription not found: " << name));
			return i->second;
		}
