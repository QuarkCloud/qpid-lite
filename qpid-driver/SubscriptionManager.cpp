...
namespace qpid {
	namespace driver {
		...
		bool SubscriptionManager::get(Message& result, const std::string& queue, sys::NanoDuration timeout) {
			return impl->get(result, queue, timeout);
		}

		Message SubscriptionManager::get(const std::string& queue, sys::NanoDuration timeout) {
