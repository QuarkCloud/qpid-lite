...
void SessionImpl::open(uint32_t timeout) // user thread
{
    ...
    if (state == INACTIVE) {
        ...
        throw sys::Exception("Open already called for this session");
    }
}
...
void SessionImpl::connectionBroke(const std::string& _text) {
    setException(sys::ExceptionHolder(new sys::TransportFailure(_text)));
    handleClosed();
}
...
void SessionImpl::sendContent(const MethodContent& content)
{
    ...
    if(data_length > 0){
        ...
        const uint32_t frag_size = (uint32_t)(maxFrameSize - AMQFrame::frameOverhead());
        ...
        if(data_length < frag_size){
            ...
            uint32_t offset = 0;
            uint32_t remaining = (uint32_t)(data_length - offset);
            while (remaining > 0) {
                ...
                offset += length;
                remaining = (uint32_t)(data_length - offset);
                if (remaining) {
...
void SessionImpl::handleClosed()
{
    demux.close(exceptionHolder.empty() ?
                sys::ExceptionHolder(new sys::ClosedException()) : exceptionHolder);
    results.close();
}
