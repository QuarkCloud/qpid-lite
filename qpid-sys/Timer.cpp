...
TimerTask::TimerTask(NanoDuration timeout, const std::string&  n) :
    name(n),
...
void Timer::run()
{
    ...
    while (active) {
        if (tasks.empty()) {
			...
			NanoDuration delay(t->sortTime, start);
            {
            ...
            if (s) {
                ...
                continue;
            } else if(NanoDuration(t->nextFireTime, start) >= 0) {
                {
				...
				NanoDuration overrun (0);
                if (!tasks.empty()) {
                    overrun = NanoDuration(tasks.top()->nextFireTime, end);
                }
                bool warningsEnabled = false;                  // TimerWarning enabled
                ...
                if (warningsEnabled) {
                    if (overrun > overran) {
                        if (delay > overran) // if delay is significant to an overrun.
                            warn.lateAndOverran(t->name, delay, overrun, NanoDuration(start, end));
                        else
                            warn.overran(t->name, overrun, NanoDuration(start, end));
                    }
                    else if (delay > late)
