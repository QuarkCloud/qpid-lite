...
namespace qpid {
	namespace amqp {
		...
		void EncodedMessage::init(qpid::amqp::MessageImpl& impl)
		{
			try {
		...
		void EncodedMessage::getReplyTo(qpid::amqp::Address& a) const
		{
		...
		bool EncodedMessage::hasHeaderChanged(const qpid::amqp::MessageImpl& msg) const
		{
			if (!durable) {
		...
		EncodedMessage::InitialScan::InitialScan(EncodedMessage& e, qpid::amqp::MessageImpl& m) : em(e), mi(m)
		{
