...
namespace qpid {
	namespace amqp {
		...
		Address::Address(const std::string& name, const std::string& subject, const Variant::Map& options,
			...
			: impl(new AddressImpl(name, subject, options)) {
			setType(type);
		}
		...
		void Address::setTemporary(Address& a, bool value){ a.impl->temporary = value; }
		bool Address::isTemporary(const Address& a){ return a.impl->temporary; }
