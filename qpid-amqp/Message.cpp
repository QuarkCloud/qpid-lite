...
//#include "qpid/amqp/amqp0_10/Codecs.h"
...
namespace qpid {
	namespace amqp {
		...
		void Message::setTtl(MilliDuration ttl) { impl->setTtl(ttl.getMilliseconds()); }
		MilliDuration Message::getTtl() const { return MilliDuration(impl->getTtl()); }
		...
		template <class C> struct MessageCodec
		{
			...
			static void decode(const Message& message, typename C::ObjectType& object, const std::string& encoding)
			{
				...
				catch (const qpid::sys::Exception &ex) {
		...
		{
			//MessageCodec<qpid::amqp::amqp0_10::MapCodec>::decode(message, map, encoding);
		}
		void decode(const Message& message, Variant::List& list, const std::string& encoding)
		{
			//MessageCodec<qpid::amqp::amqp0_10::ListCodec>::decode(message, list, encoding);
		}
		void encode(const Variant::Map& map, Message& message, const std::string& encoding)
		{
			//MessageCodec<qpid::amqp::amqp0_10::MapCodec>::encode(map, message, encoding);
		}
		void encode(const Variant::List& list, Message& message, const std::string& encoding)
		{
			//MessageCodec<qpid::amqp::amqp0_10::ListCodec>::encode(list, message, encoding);
		}
