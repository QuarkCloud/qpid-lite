	...
	namespace amqp {
		...
		void MessageReader::HeaderReader::onUInt(uint32_t v, const Descriptor*) // ttl, delivery-count
		{
			if (index == TTL) {
				...
				QPID_LOG(warning, "Unexpected message format, got uint32_t at index " << index << " of headers");
			}
			++index;
		}
		...
		void MessageReader::PropertiesReader::onUInt(uint32_t v, const Descriptor*) // group-sequence
		{
			if (index == GROUP_SEQUENCE) {
				...
				QPID_LOG(warning, "Unexpected message format, got uint32_t at index " << index << " of properties");
			}
			++index;
		}
		...
		void MessageReader::onUInt(uint32_t v, const Descriptor* descriptor)
		{
			if (delegate) {
				...
				if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
					...
					if (!descriptor) {
						QPID_LOG(warning, "Expected described type but got uint32_t value with no descriptor.");
					}
					else {
						QPID_LOG(warning, "Unexpected uint32_t value with descriptor: " << *descriptor);
					}
				}
			}
		}
