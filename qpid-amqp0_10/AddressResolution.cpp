...
namespace qpid {
	namespace amqp0_10 {
		...
		using qpid::amqp::AddressError;
		using qpid::amqp::MalformedAddress;
		using qpid::amqp::ResolutionError;
		using qpid::amqp::NotFound;
		using qpid::amqp::AssertionFailed;
			...
			alternateExchange((Opt(a) / NODE / X_DECLARE / ALTERNATE_EXCHANGE).str())
		{
			...
			if (qpid::amqp::Address::isTemporary(a) && createPolicy.isVoid()) {
				createPolicy = "always";
