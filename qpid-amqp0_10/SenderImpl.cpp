...
namespace qpid {
	namespace amqp0_10 {
		...
		SenderImpl::SenderImpl(SessionImpl& _parent, const std::string& _name,
			const qpid::amqp::Address& _address, bool _autoReconnect) :
		...
		qpid::amqp::Address SenderImpl::getAddress() const
		{
			return address;
		}

		void SenderImpl::send(const qpid::amqp::Message& message, bool sync)
		{
		...
		void SenderImpl::waitForCapacity()
		{
			...
			try {
				...
				throw qpid::amqp::OutOfCapacity(name);
			}
		}

		void SenderImpl::sendImpl(const qpid::amqp::Message& m)
		{
			sys::Mutex::ScopedLock l(lock);
		...
		void SenderImpl::sendUnreliable(const qpid::amqp::Message& m)
		{
			sys::Mutex::ScopedLock l(lock);
			OutgoingMessage msg;
			msg.setSubject(m.getSubject().empty() ? address.getSubject() : m.getSubject());
