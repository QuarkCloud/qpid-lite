...
namespace qpid {
	namespace amqp0_10 {
		...
		void OutgoingMessage::convert(const qpid::amqp::Message& from)
		{
			//TODO: need to avoid copying as much as possible
			if (from.getContentObject().getType() == qpid::types::VAR_MAP) {
				std::string content;
				qpid::amqp0_10::MapCodec::encode(from.getContentObject().asMap(), content);
				message.getMessageProperties().setContentType(qpid::amqp0_10::MapCodec::contentType);
				message.setData(content);
			}
			else if (from.getContentObject().getType() == qpid::types::VAR_LIST) {
				std::string content;
				qpid::amqp0_10::ListCodec::encode(from.getContentObject().asList(), content);
				message.getMessageProperties().setContentType(qpid::amqp0_10::ListCodec::contentType);
				message.setData(content);
			}
		...
		void OutgoingMessage::markRedelivered()
		{
			...
			if (message.getDeliveryProperties().hasTtl()) {
				uint64_t delta = qpid::sys::NanoDuration(base, qpid::sys::now()) / qpid::sys::TIME_MSEC;
