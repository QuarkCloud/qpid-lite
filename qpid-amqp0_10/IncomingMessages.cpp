...
namespace qpid {
	namespace amqp0_10 {
		...
		namespace {
			qpid::sys::NanoDuration get_duration(qpid::sys::NanoDuration timeout, qpid::sys::AbsTime deadline)
			{
				if (timeout == qpid::sys::TIME_INFINITE) {
					return qpid::sys::TIME_INFINITE;
				}
				else {
					return std::max(qpid::sys::NanoDuration(0), qpid::sys::NanoDuration(AbsTime::now(), deadline));
				}
			}
		}

		bool IncomingMessages::get(Handler& handler, qpid::sys::NanoDuration timeout)
		{
			sys::Mutex::ScopedLock l(lock);
			...
			do {
				...
				if (handler.isClosed()) throw qpid::amqp::ReceiverError("Receiver has been closed");
		...
		bool IncomingMessages::getNextDestination(std::string& destination, qpid::sys::NanoDuration timeout)
		{
			sys::Mutex::ScopedLock l(lock);
			AbsTime deadline(AbsTime::now(), timeout);
		...
		bool IncomingMessages::pop(FrameSet::shared_ptr& content, qpid::sys::NanoDuration timeout)
		{
			try {
		 ...
		 * for later retrieval.
		 */
		IncomingMessages::ProcessState IncomingMessages::process(Handler* handler, qpid::sys::NanoDuration duration)
		{
			AbsTime deadline(AbsTime::now(), duration);
			FrameSet::shared_ptr content;
			try {
				for (NanoDuration timeout = duration; pop(content, timeout); timeout = NanoDuration(AbsTime::now(), deadline)) {
			...
			catch (const qpid::sys::ClosedException&) { return CLOSED; }
			return EMPTY;
		}

		bool IncomingMessages::wait(qpid::sys::NanoDuration duration)
		{
			AbsTime deadline(AbsTime::now(), duration);
			FrameSet::shared_ptr content;
			for (NanoDuration timeout = duration; pop(content, timeout); timeout = NanoDuration(AbsTime::now(), deadline)) {
		...
		void populate(qpid::amqp::Message& message, FrameSet& command);
		 ...
		 */
		void IncomingMessages::retrieve(FrameSetPtr command, qpid::amqp::Message* message)
		{
			if (message) {
		...
		void IncomingMessages::MessageTransfer::retrieve(qpid::amqp::Message* message)
		{
		...
		void populateHeaders(qpid::amqp::Message& message,
			const DeliveryProperties* deliveryProperties,
			const MessageProperties* messageProperties)
		{
			if (deliveryProperties) {
				message.setTtl(qpid::amqp::MilliDuration(deliveryProperties->getTtl()));
		...
		void populateHeaders(qpid::amqp::Message& message, const AMQHeaderBody* headers)
		{
