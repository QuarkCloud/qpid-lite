 ...
 *
 */
#include "qpid/messaging/amqp/ConnectionContext.h"
#include "qpid/messaging/amqp/DriverImpl.h"
...
#include "qpid/messaging/amqp/SessionContext.h"
...
boost::shared_ptr<SenderContext> ConnectionContext::createSender(boost::shared_ptr<SessionContext> session, const qpid::messaging::Address& address)
{
    ...
    boost::shared_ptr<SenderContext> sender =
        session->createSender(usePassthrough() ? passthrough(address) : address,
                              SenderOptions(setToOnSend, maxDeliveryAttempts, raiseRejected, 
								  (uint64_t)(redeliveryTimeout * (double)qpid::sys::TIME_SEC)));
    try {
...
void ConnectionContext::checkClosed(boost::shared_ptr<SessionContext> ssn, pn_link_t* lnk)
{
    ...
    if ((pn_link_state(lnk) & REQUIRES_CLOSE) == REQUIRES_CLOSE) {
        ...
        if (name == qpid::messaging::amqp::error_conditions::NOT_FOUND) {
            throw qpid::messaging::NotFound(text);
        } else if (name == qpid::messaging::amqp::error_conditions::UNAUTHORIZED_ACCESS) {
...
void ConnectionContext::startTxSession(boost::shared_ptr<SessionContext> session) {
    try {
        ...
        throw messaging::TransactionError(sys::Msg() << "Cannot start transaction: " << e.what());
    }
}
