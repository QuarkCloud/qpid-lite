...
using namespace qpid::messaging::amqp::message;
		...
		namespace amqp {
			...
			bool MessageReader::onStartList(uint32_t count, const CharSequence& elements, const CharSequence& raw, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (!descriptor) {
						...
						onAmqpValue(elements, qpid::messaging::amqp::typecodes::LIST_NAME, nested(descriptor));
						return false;
					}
					else {
			...
			bool MessageReader::onStartMap(uint32_t count, const CharSequence& elements, const CharSequence& raw, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (!descriptor) {
						...
						onAmqpValue(elements, qpid::messaging::amqp::typecodes::MAP_NAME, nested(descriptor));
						return false;
					}
					else {
			...
			void MessageReader::onBinary(const CharSequence& bytes, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (!descriptor) {
						...
						onAmqpValue(bytes, qpid::messaging::amqp::typecodes::BINARY_NAME, nested(descriptor));
					}
					else {
			...
			void MessageReader::onString(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(AMQP_VALUE_SYMBOL, AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::messaging::amqp::typecodes::STRING_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			void MessageReader::onSymbol(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(AMQP_VALUE_SYMBOL, AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::messaging::amqp::typecodes::SYMBOL_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			void MessageReader::onUuid(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(AMQP_VALUE_SYMBOL, AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::messaging::amqp::typecodes::UUID_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			bool MessageReader::onStartArray(uint32_t count, const CharSequence& raw, const Constructor& constructor, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(AMQP_VALUE_SYMBOL, AMQP_VALUE_CODE)) {
						...
						onAmqpValue(raw, qpid::messaging::amqp::typecodes::ARRAY_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
