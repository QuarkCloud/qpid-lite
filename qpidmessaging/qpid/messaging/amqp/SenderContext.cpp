...
class HeaderAdapter : public qpid::messaging::amqp::MessageEncoder::Header
{
  public:
...
class PropertiesAdapter : public qpid::messaging::amqp::MessageEncoder::Properties
{
  public:
...
class ApplicationPropertiesAdapter : public qpid::messaging::amqp::MessageEncoder::ApplicationProperties
{
  public:
    ApplicationPropertiesAdapter(const qpid::types::Variant::Map& h) : headers(h) {}
    void handle(qpid::messaging::amqp::MapHandler& h) const
    {
        for (qpid::types::Variant::Map::const_iterator i = headers.begin(); i != headers.end(); ++i) {
            ...
            if (!startsWith(i->first, X_AMQP) || i->first == X_AMQP_0_10_APP_ID) {
                qpid::messaging::amqp::CharSequence key(convert(i->first));
  ...
  private:
    ...
    static qpid::messaging::amqp::CharSequence convert(const std::string& in)
    {
        qpid::messaging::amqp::CharSequence out;
        out.data = in.data();
        out.size = in.size();
        return out;
    }
    ...
    try {
        ...
        if (original && !changedSubject(msg, address)) { //still have the content as received, send at least the bare message unaltered
            ...
            if (original->hasHeaderChanged(msg)) {
                ...
                qpid::messaging::amqp::MessageEncoder encoder(encoded.getData(), encoded.getSize());
            ...
            //compute size:
            size_t contentSize = qpid::messaging::amqp::MessageEncoder::getEncodedSize(header)
                + qpid::messaging::amqp::MessageEncoder::getEncodedSize(properties)
                + qpid::messaging::amqp::MessageEncoder::getEncodedSize(applicationProperties);
            if (msg.getContent().isVoid()) {
                contentSize += qpid::messaging::amqp::MessageEncoder::getEncodedSizeForContent(msg.getBytes());
            } else {
                contentSize += qpid::messaging::amqp::MessageEncoder::getEncodedSizeForValue(msg.getContent()) + 3/*descriptor*/;
            }
            ...
            if (!msg.getContent().isVoid()) {
                //write as AmqpValue
                encoder.writeValue(msg.getContent(), &qpid::messaging::amqp::message::AMQP_VALUE);
            } else if (msg.getBytes().size()) {
                encoder.writeBinary(msg.getBytes(), &qpid::messaging::amqp::message::DATA);//structured content not yet directly supported
            }
...
void SenderContext::Delivery::send(pn_link_t* sender, bool unreliable, const types::Variant& state)
{
    ...
    if (!state.isVoid()) {      // Add transaction state
        ...
        data.put(state);
        pn_delivery_update(token, qpid::messaging::amqp::transaction::TRANSACTIONAL_STATE_CODE);
    }
...
bool SenderContext::Delivery::delivered()
{
    if (settled) {
        ...
        if (delivery_refused()) {
            throw MessageRejected(sys::Msg() << "delivery " << id << " refused: " << getStatus());
        } else if (not_delivered()) {
            if (max_attempts && (attempts >= max_attempts)) {
                throw MessageRejected(sys::Msg() << "delivery " << id << " cannot be delivered after " << attempts << " attempts");
            } else if (qpid::sys::now() > retry_until) {
                throw MessageRejected(sys::Msg() << "delivery " << id << " cannot be delivered, timed out after " << attempts << " attempts");
            } else {
                ...
                throw MessageReleased(sys::Msg() << "delivery " << id << " was not successful: " << status);
            }
        } else if (!accepted()) {
    ...
    if (rejected()) {
        ...
        if (d) {
            ...
            if (c && pn_condition_is_set(c)) {
                return sys::Msg() << pn_condition_get_name(c) << ": " << pn_condition_get_description(c);
            }
        }
        return "rejected";
    } else if (released()) {
...
std::string SenderContext::Delivery::error()
{
    ...
    return (condition && pn_condition_is_set(condition)) ?
		sys::Msg() << get_error_string(condition, std::string(), std::string()) :
        std::string();
}
