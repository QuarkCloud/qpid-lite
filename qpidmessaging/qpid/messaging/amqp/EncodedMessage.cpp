...
#include "qpid/messaging/amqp/Decoder.h"
#include "qpid/messaging/amqp/DataBuilder.h"
#include "qpid/messaging/amqp/ListBuilder.h"
#include "qpid/messaging/amqp/MapBuilder.h"
#include "qpid/messaging/amqp/typecodes.h"
...
void EncodedMessage::init(qpid::messaging::MessageImpl& impl)
{
    try {
        ...
        qpid::messaging::amqp::Decoder decoder(data, size);
...
void EncodedMessage::populate(qpid::types::Variant::Map& map) const
{
    try {
        ...
        if (applicationProperties) {
            qpid::messaging::amqp::Decoder decoder(applicationProperties.data, applicationProperties.size);
            decoder.readMap(map);
        }
        ...
        if (deliveryAnnotations) {
            qpid::messaging::amqp::Decoder decoder(deliveryAnnotations.data, deliveryAnnotations.size);
            if (nestAnnotations) {
        ...
        if (messageAnnotations) {
            qpid::messaging::amqp::Decoder decoder(messageAnnotations.data, messageAnnotations.size);
            if (nestAnnotations) {
...
qpid::messaging::amqp::CharSequence EncodedMessage::getBareMessage() const
{
    return bareMessage;
}
...
void EncodedMessage::getBody(std::string& raw, qpid::types::Variant& c) const
{
    try {
        if (!content.isVoid()) {
            ...
            if (bodyType.empty()
                || bodyType == qpid::messaging::amqp::typecodes::BINARY_NAME
            ...
            } else if (bodyType == qpid::messaging::amqp::typecodes::LIST_NAME) {
                qpid::messaging::amqp::ListBuilder builder;
                qpid::messaging::amqp::Decoder decoder(body.data, body.size);
                decoder.read(builder);
                c = builder.getList();
                raw.assign(body.data, body.size);
            } else if (bodyType == qpid::messaging::amqp::typecodes::MAP_NAME) {
                qpid::types::Variant v = qpid::types::Variant::Map();
                qpid::messaging::amqp::DataBuilder builder(v);
                qpid::messaging::amqp::Decoder decoder(body.data, body.size);
                decoder.read(builder);
                c = builder.getValue().asMap();
                raw.assign(body.data, body.size);
            } else if (bodyType == qpid::messaging::amqp::typecodes::UUID_NAME) {
                if (body.size == qpid::types::Uuid::SIZE) c = qpid::types::Uuid(body.data);
                raw.assign(body.data, body.size);
            } else if (bodyType == qpid::messaging::amqp::typecodes::ARRAY_NAME) {
                raw.assign(body.data, body.size);
            }
        }
    } catch (const qpid::sys::Exception& e) {
...
qpid::messaging::amqp::CharSequence EncodedMessage::getBody() const
{
    return body;
}
...
void EncodedMessage::InitialScan::onMessageId(const qpid::messaging::amqp::CharSequence& v, qpid::types::VariantType t) { em.messageId.set(v, t); }
void EncodedMessage::InitialScan::onUserId(const qpid::messaging::amqp::CharSequence& v) { em.userId = v; }
void EncodedMessage::InitialScan::onTo(const qpid::messaging::amqp::CharSequence& v) { em.to = v; }
void EncodedMessage::InitialScan::onSubject(const qpid::messaging::amqp::CharSequence& v) { em.subject = v; }
void EncodedMessage::InitialScan::onReplyTo(const qpid::messaging::amqp::CharSequence& v) { em.replyTo = v;}
void EncodedMessage::InitialScan::onCorrelationId(uint64_t v) { em.correlationId.set(v); }
void EncodedMessage::InitialScan::onCorrelationId(const qpid::messaging::amqp::CharSequence& v, qpid::types::VariantType t) { em.correlationId.set(v, t); }
void EncodedMessage::InitialScan::onContentType(const qpid::messaging::amqp::CharSequence& v) { em.contentType = v; }
void EncodedMessage::InitialScan::onContentEncoding(const qpid::messaging::amqp::CharSequence& v) { em.contentEncoding = v; }
...
void EncodedMessage::InitialScan::onGroupId(const qpid::messaging::amqp::CharSequence& v) { em.groupId = v; }
void EncodedMessage::InitialScan::onGroupSequence(uint32_t i) { em.groupSequence = i; }
void EncodedMessage::InitialScan::onReplyToGroupId(const qpid::messaging::amqp::CharSequence& v) { em.replyToGroupId = v; }

void EncodedMessage::InitialScan::onApplicationProperties(const qpid::messaging::amqp::CharSequence& v, const qpid::messaging::amqp::CharSequence&) { em.applicationProperties = v; }
void EncodedMessage::InitialScan::onDeliveryAnnotations(const qpid::messaging::amqp::CharSequence& v, const qpid::messaging::amqp::CharSequence&) { em.deliveryAnnotations = v; }
void EncodedMessage::InitialScan::onMessageAnnotations(const qpid::messaging::amqp::CharSequence& v, const qpid::messaging::amqp::CharSequence&) { em.messageAnnotations = v; }

void EncodedMessage::InitialScan::onData(const qpid::messaging::amqp::CharSequence& v)
{
    em.body = v;
}
void EncodedMessage::InitialScan::onAmqpSequence(const qpid::messaging::amqp::CharSequence& v)
{
    em.body = v;
    em.bodyType = qpid::messaging::amqp::typecodes::LIST_NAME;
}
void EncodedMessage::InitialScan::onAmqpValue(const qpid::messaging::amqp::CharSequence& v, const std::string& type, const qpid::messaging::amqp::Descriptor*)
{
    em.body = v;
    if (type == qpid::messaging::amqp::typecodes::STRING_NAME) {
        em.bodyType = qpid::types::encodings::UTF8;
    } else if (type == qpid::messaging::amqp::typecodes::SYMBOL_NAME) {
        em.bodyType = qpid::types::encodings::ASCII;
    } else {
        em.bodyType = type;
    }
}
void EncodedMessage::InitialScan::onAmqpValue(const qpid::types::Variant& v, const qpid::messaging::amqp::Descriptor*)
{
    em.content = v;
}

void EncodedMessage::InitialScan::onFooter(const qpid::messaging::amqp::CharSequence& v, const qpid::messaging::amqp::CharSequence&) { em.footer = v; }
