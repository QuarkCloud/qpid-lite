...
#include "qpid/messaging/amqp/MapEncoder.h"
#include "qpid/messaging/amqp/MapSizeCalculator.h"
	...
	namespace messaging {
		namespace amqp {
			...
			void MessageEncoder::writeHeader(const Header& msg)
			{
				...
				if (fields) {
					void* token = startList8(&qpid::messaging::amqp::message::HEADER);
			...
			void MessageEncoder::writeProperties(const Properties& msg)
			{
				...
				if (fields) {
					void* token = startList32(&qpid::messaging::amqp::message::PROPERTIES);
			...
			void MessageEncoder::writeApplicationProperties(const ApplicationProperties& properties)
			{
				...
				size_t required = calc.getTotalSizeRequired(&qpid::messaging::amqp::message::APPLICATION_PROPERTIES);
				assert(required <= getSize() - getPosition());
				MapEncoder encoder(skip(required), required);
				encoder.writeMetaData(calc.getSize(), calc.getCount() * 2, &qpid::messaging::amqp::message::APPLICATION_PROPERTIES);
			...
			{
				writeMap(properties, &qpid::messaging::amqp::message::APPLICATION_PROPERTIES, large);
			}
			...
			size_t MessageEncoder::getEncodedSize(const ApplicationProperties& p)
			{
				MapSizeCalculator calc;
				p.handle(calc);
				return calc.getTotalSizeRequired(&qpid::messaging::amqp::message::APPLICATION_PROPERTIES);
			}
