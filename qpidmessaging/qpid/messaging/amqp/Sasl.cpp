...
Sasl::Sasl(const std::string& id, ConnectionContext& c, const std::string& hostname_)
    : qpid::messaging::amqp::SaslClient(id), context(c),
      sasl(qpid::sys::SaslFactory::getInstance().create(c.username, c.password, c.service, hostname_, c.minSsf, c.maxSsf, false)),
