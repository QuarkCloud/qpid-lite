 ...
 *
 */
#include "qpid/messaging/amqp/MapReader.h"
...
namespace qpid {
	namespace messaging {
		namespace amqp {
			...
			{
				if (!level) throw qpid::sys::Exception(QPID_MSG("Expecting map as top level datum"));
				if (key) {
					onNullValue(key, d);
					clearKey();
				}
				else {
					throw qpid::sys::Exception(QPID_MSG("Expecting symbol as key"));
				}
			}
			...
			void MapReader::onString(const CharSequence& v, const Descriptor* d)
			{
				...
				if (key) {
					...
					if (keyType & STRING_KEY) {
						...
						throw qpid::sys::Exception(QPID_MSG("Expecting symbol as key, got string " << v.str()));
					}
				}
			}
			...
			void MapReader::onSymbol(const CharSequence& v, const Descriptor* d)
			{
				...
				if (key) {
					...
					if (keyType & SYMBOL_KEY) {
						...
						throw qpid::sys::Exception(QPID_MSG("Expecting string as key, got symbol " << v.str()));
					}
				}
			}
