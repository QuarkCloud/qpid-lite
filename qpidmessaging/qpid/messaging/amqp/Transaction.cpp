...
void Transaction::declare(SendFunction send, const SessionPtr& session) {
    ...
    error.raise();
    clear();
    Variant declare = Variant::described(qpid::messaging::amqp::transaction::DECLARE_CODE, Variant::List());
...
void Transaction::discharge(SendFunction send, const SessionPtr& session, bool fail) {
    ...
    try {
        ...
        discharge.setDescriptor(qpid::messaging::amqp::transaction::DISCHARGE_CODE);
        SenderContext::Delivery* delivery = 0;
...
void Transaction::setId(const SenderContext::Delivery& delivery)
{
    if (delivery.getToken() &&
        pn_delivery_remote_state(delivery.getToken()) == qpid::messaging::amqp::transaction::DECLARED_CODE)
    {
...
void Transaction::setId(const std::string& id_) {
    ...
    if (id.empty()) {
        ...
        Variant accepted = Variant::described(qpid::messaging::amqp::message::ACCEPTED_CODE, Variant::List());
...
void Transaction::acknowledge(pn_delivery_t* delivery)
{
    ...
    pn_delivery_update(delivery, qpid::messaging::amqp::transaction::TRANSACTIONAL_STATE_CODE);
