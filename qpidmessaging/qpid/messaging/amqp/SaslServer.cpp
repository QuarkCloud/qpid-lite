 ...
 *
 */
#include "qpid/messaging/amqp/SaslServer.h"
...
using namespace qpid::messaging::amqp::sasl;
			...
			void SaslServer::mechanisms(const std::string& mechanisms)
			{
				...
				if (parts.size() > 1) {
					...
					for (std::vector<std::string>::const_iterator i = parts.begin(); i != parts.end(); ++i) {
						uint8_t size = i->size() > std::numeric_limits<uint8_t>::max() ? 
							std::numeric_limits<uint8_t>::max() : (uint8_t)i->size();
						encoder.write(size);
