...
#include "qpid/messaging/amqp0_10/ConnectionImpl.h"
...
namespace messaging {
namespace {
struct ProtocolOptions : qpid::sys::Options
{
    std::string protocolDefaults;

    ProtocolOptions() : qpid::sys::Options("Protocol Settings")
    {
        addOptions()
            ("protocol-defaults", sys::optValue(protocolDefaults, "PROTOCOLS"), "Protocols to use when none are specified");
    }
};
...
{
    return new qpid::messaging::amqp0_10::ConnectionImpl(url, options);
}
  ...
  public:
    Registry()
    {
        ...
        if (!options.protocolDefaults.empty()) {
			sys::split(versions, options.protocolDefaults, ", ");
        }
    }
...
ConnectionImpl* ProtocolRegistry::create(const std::string& url, const Variant::Map& options)
{
    ...
    if (extract("protocol", name, options, stripped)) {
		sys::split(versions, name.asString(), ", ");
    } else {
