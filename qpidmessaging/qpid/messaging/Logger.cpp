...
try
{
    ...
    qpid::sys::Options myOptions;
    myOptions.addOptions()
    ((prefix+"log-enable").c_str(), sys::optValue(selectors, "RULE"),
    ...
    ((prefix+"log-disable").c_str(), sys::optValue(deselectors, "RULE"),
     ...
     "This option can be used multiple times").c_str())
    ((prefix+"log-time").c_str(), sys::optValue(time, "yes|no"), "Include time in log messages")
    ((prefix+"log-level").c_str(), sys::optValue(level,"yes|no"), "Include severity level in log messages")
    ((prefix+"log-source").c_str(), sys::optValue(source,"yes|no"), "Include source file:line in log messages")
    ((prefix+"log-thread").c_str(), sys::optValue(thread,"yes|no"), "Include thread ID in log messages")
    ((prefix+"log-function").c_str(), sys::optValue(function,"yes|no"), "Include function signature in log messages")
    ((prefix+"log-hires-timestamp").c_str(), sys::optValue(hiresTs,"yes|no"), "Use hi-resolution timestamps in log messages")
    ((prefix+"log-to-stderr").c_str(), sys::optValue(logToStderr, "yes|no"), "Send logging output to stderr")
    ((prefix+"log-to-stdout").c_str(), sys::optValue(logToStdout, "yes|no"), "Send logging output to stdout")
    ((prefix+"log-to-file").c_str(), sys::optValue(logFile, "FILE"), "Send log output to FILE.")
    ;
