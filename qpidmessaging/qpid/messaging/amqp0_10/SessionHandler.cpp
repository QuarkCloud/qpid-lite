...
#include "qpid/messaging/amqp0_10/SessionHandler.h"
	...
	namespace messaging {
		namespace amqp0_10 {
			...
			void SessionHandler::handleIn(AMQFrame& f) {
				...
				catch (const framing::SessionException& e) {
			...
			void SessionHandler::handleException(const qpid::framing::SessionException& e)
			{
        ...
        throw InvalidArgumentException(                                 \
			...
			void SessionHandler::detached(const std::string& /*name*/, uint8_t code) {
				...
				if (code != session::DETACH_CODE_NORMAL) {
					...
					channelException(convert(code), sys::Msg() << "Channel " << channel.get()
