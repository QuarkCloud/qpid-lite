 ...
 *
 */
#include "qpid/messaging/amqp0_10/ReceiverImpl.h"
#include "qpid/messaging/amqp0_10/AddressResolution.h"
#include "qpid/messaging/amqp0_10/MessageSource.h"
#include "qpid/messaging/amqp0_10/SessionImpl.h"
...
bool ReceiverImpl::getImpl(qpid::messaging::Message& message, qpid::messaging::Duration timeout)
{
    ...
    if (parent->get(*this, message, timeout)) {
        if (autoDecode) {
            if (message.getContentType() == qpid::messaging::amqp0_10::MapCodec::contentType) {
                message.getContentObject() = qpid::types::Variant::Map();
                decode(message, message.getContentObject().asMap());
            } else if (message.getContentType() == qpid::messaging::amqp0_10::ListCodec::contentType) {
