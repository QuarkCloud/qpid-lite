 ...
 *
 */
#include "qpid/messaging/amqp0_10/OutgoingMessage.h"
#include "qpid/messaging/amqp0_10/AddressResolution.h"
...
using namespace qpid::messaging::amqp0_10;
...
void OutgoingMessage::convert(const qpid::messaging::Message& from)
{
    ...
    if (from.getContentObject().getType() == qpid::types::VAR_MAP) {
        std::string content;
        qpid::messaging::amqp0_10::MapCodec::encode(from.getContentObject().asMap(), content);
        message.getMessageProperties().setContentType(qpid::messaging::amqp0_10::MapCodec::contentType);
        message.setData(content);
    } else if (from.getContentObject().getType() == qpid::types::VAR_LIST) {
        std::string content;
        qpid::messaging::amqp0_10::ListCodec::encode(from.getContentObject().asList(), content);
        message.getMessageProperties().setContentType(qpid::messaging::amqp0_10::ListCodec::contentType);
        message.setData(content);
