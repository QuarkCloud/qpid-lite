...
#include "qpid/messaging/amqp0_10/CodecsInternal.h"
...
namespace qpid {
	namespace messaging {
		namespace amqp0_10 {
			...
			{
				uint32_t len = qpid::messaging::amqp0_10::encodedSize(value);
				std::vector<char> space(len);
				qpid::framing::Buffer buff(&space[0], len);

				qpid::messaging::amqp0_10::encode(value, len, buff);
			...
			{
				return qpid::messaging::amqp0_10::encodedSize(value);
			}
