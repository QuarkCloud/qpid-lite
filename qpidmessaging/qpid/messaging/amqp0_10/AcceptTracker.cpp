 ...
 *
 */
#include "qpid/messaging/amqp0_10/AcceptTracker.h"
...
qpid::framing::SequenceSet AcceptTracker::State::accept(qpid::framing::SequenceNumber id, bool cumulative)
{
	qpid::framing::SequenceSet accepting;
    if (cumulative) {
        for (qpid::framing::SequenceSet::iterator i = unaccepted.begin(); i != unaccepted.end() && *i <= id; ++i) {
            accepting.add(*i);
        }
