...
template <>
QPID_FRAMING_EXTERN uint64_t Buffer::getUInt<1>() {
    return getOctet();
}

template <>
QPID_FRAMING_EXTERN uint64_t Buffer::getUInt<2>() {
    return getShort();
}

template <>
QPID_FRAMING_EXTERN uint64_t Buffer::getUInt<4>() {
    return getLong();
}

template <>
QPID_FRAMING_EXTERN uint64_t Buffer::getUInt<8>() {
    return getLongLong();
}

template <>
QPID_FRAMING_EXTERN void Buffer::putUInt<1>(uint64_t i) {
    if (std::numeric_limits<uint8_t>::min() <= i && i <= std::numeric_limits<uint8_t>::max()) {
        putOctet((uint8_t)i);
        return;
    }
    throw sys::Exception(QPID_MSG("Could not encode (" << i << ") as uint8_t."));
}

template <>
QPID_FRAMING_EXTERN void Buffer::putUInt<2>(uint64_t i) {
    if (std::numeric_limits<uint16_t>::min() <= i && i <= std::numeric_limits<uint16_t>::max()) {
        putShort((uint16_t)i);
        return;
    }
    throw sys::Exception(QPID_MSG("Could not encode (" << i << ") as uint16_t."));
}

template <>
QPID_FRAMING_EXTERN void Buffer::putUInt<4>(uint64_t i) {
    if (std::numeric_limits<uint32_t>::min() <= i && i <= std::numeric_limits<uint32_t>::max()) {
        putLong((uint32_t)i);
        return;
    }
    throw sys::Exception(QPID_MSG("Could not encode (" << i << ") as uint32_t."));
}

template <>
QPID_FRAMING_EXTERN void Buffer::putUInt<8>(uint64_t i) {
    putLongLong(i);
}
...
void Buffer::putShortString(const string& s){
    ...
    throw sys::Exception(QPID_MSG("Could not encode string of " << slen << " bytes as uint8_t string."));
}
...
void Buffer::putMediumString(const string& s){
    ...
    throw sys::Exception(QPID_MSG("Could not encode string of " << slen << " bytes as uint16_t string."));
}
...
void Buffer::putLongString(const string& s){
    ...
    throw sys::Exception(QPID_MSG("Could not encode string of " << slen << " bytes as uint32_t string."));
}
