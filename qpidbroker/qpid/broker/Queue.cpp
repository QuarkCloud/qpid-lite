    ...
    redirectSource(false)
{
    ...
    if (settings.traceExcludes.size()) {
     ...
     qpid::broker::amqp0_10::translate(settings.asMap(), encodableSettings);
...
bool Queue::accept(const Message& msg)
{
    ...
    if (broker::amqp0_10::MessageTransfer::isImmediateDeliveryRequired(msg) && getConsumerCount() == 0) {
...
namespace {
    ...
    MessageFilter* MessageFilter::create( const ::qpid::types::Variant::Map *filter )
    {
        ...
        if (filter && !filter->empty()) {
            ...
            throw qpid::sys::Exception(QPID_MSG("Unrecognized message filter: '" << *filter << "'"));
        }
...
void Queue::addArgument(const string& key, const types::Variant& value) {
    ...
    boost::shared_ptr<qpid::framing::FieldValue> v;
     qpid::broker::amqp0_10::translate(value, v);
