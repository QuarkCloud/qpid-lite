    ...
    sessionName("qpid.bridge_session_" + name + "_" + link->getBroker()->getFederationTag())
{
    ...
    if (args.i_credit && args.i_sync && args.i_sync > args.i_credit)
        throw qpid::sys::Exception("The credit value must be greater than configured sync (ack) interval.");
...
void Bridge::create(amqp0_10::Connection& c)
{
    ...
    if (args.i_srcIsLocal) {
        if (args.i_dynamic)
            throw qpid::sys::Exception("Dynamic routing not supported for push routes");
    ...
    if (initialize) {
        ...
        if (args.i_srcIsQueue) {
            ...
            if (args.i_dynamic) {
                ...
                if (exchange.get() == 0)
                    throw qpid::sys::Exception("Exchange not found for dynamic route");
...
void Bridge::cancel(amqp0_10::Connection& c)
{
...
qmf2::ManagementObject::shared_ptr Bridge::GetManagementObject(void) const
{
    return mgmtObject;
}

qmf2::Manageable::status_t Bridge::ManagementMethod(uint32_t methodId,
                                                          qmf2::Args& /*args*/,
                                                          string&)
{
    if (methodId == _qmf::Bridge::METHOD_CLOSE) {
        ...
        close();
        return qmf2::Manageable::STATUS_OK;
    } else {
        return qmf2::Manageable::STATUS_UNKNOWN_METHOD;
    }
}
