    ...
    protocols(parent->getSession().getBroker().getProtocolRegistry())
{
    if (parent != 0 && queue.get() != 0 && queue->GetManagementObject() !=0)
    {
        ...
        qmf2::Manageable* ms = dynamic_cast<qmf2::Manageable*> (&(parent->session));
...
{
    allocateCredit(msg);
    boost::intrusive_ptr<const amqp0_10::MessageTransfer> transfer = protocols.translate(msg);
    DeliveryRecord record(cursor, msg.getSequence(), (uint32_t)msg.getReplicationId(), queue, getTag(),
...
void SemanticState::route(Message& msg, Deliverable& strategy) {
    std::string exchangeName = qpid::broker::amqp0_10::MessageTransfer::get(msg).getExchangeName();
