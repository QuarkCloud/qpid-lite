...
string  TopicPolicy::packageName  = string ("org.apache.qpid.broker");
string  TopicPolicy::className    = string ("topicpolicy");
uint8_t TopicPolicy::md5Sum[MD5_LEN]   =
    {0x36,0x53,0x24,0x1b,0x4c,0x9c,0xcf,0x12,0x72,0xfb,0xaa,0xa8,0x6,0xb1,0x4e,0x8};

TopicPolicy::TopicPolicy (ManagementAgent*, Manageable* _core, const std::string& _name) :
...
void TopicPolicy::debugStats (const std::string& comment)
{
...
void TopicPolicy::registerSelf(ManagementAgent* agent)
{
    agent->registerClass(packageName, className, md5Sum, writeSchema);
}

void TopicPolicy::writeSchema (std::string& schema)
{
...
uint32_t TopicPolicy::writePropertiesSize() const
{
...
void TopicPolicy::readProperties (const std::string& _sBuf)
{
...
void TopicPolicy::writeProperties (std::string& _sBuf) const
{
    const int _bufSize=65536;
    char _msgChars[_bufSize];
    ::qmf2::Buffer buf(_msgChars, _bufSize);
...
void TopicPolicy::writeStatistics (std::string& _sBuf, bool skipHeaders)
{
    const int _bufSize=65536;
    char _msgChars[_bufSize];
    ::qmf2::Buffer buf(_msgChars, _bufSize);
...
void TopicPolicy::doMethod (string&, const string&, string& outStr, const string&)
{
...
std::string TopicPolicy::getKey() const
{
...
void TopicPolicy::mapEncodeValues (::qpid::types::Variant::Map& _map,
...
void TopicPolicy::mapDecodeValues (const ::qpid::types::Variant::Map& _map)
{
...
void TopicPolicy::doMethod (string&, const ::qpid::types::Variant::Map&, ::qpid::types::Variant::Map& outMap, const string&)
{
    Manageable::status_t status = Manageable::STATUS_UNKNOWN_METHOD;
    std::string          text;
