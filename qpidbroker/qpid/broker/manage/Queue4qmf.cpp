...
#include "qpid/broker/manage/ArgsQueuePurge.h"
#include "qpid/broker/manage/ArgsQueueReroute.h"
...
uint8_t Queue::md5Sum[MD5_LEN]   =
    {0x84,0xce,0x32,0x6e,0x2c,0xbd,0x68,0x99,0x80,0xd7,0x62,0x67,0xd3,0x32,0x7e,0xf};

Queue::Queue (ManagementAgent*, Manageable* _core, ::qmf2::Manageable* _parent, const std::string& _name, bool _durable, bool _autoDelete) :
    ManagementObject(_core),name(_name),durable(_durable),autoDelete(_autoDelete)
{
    vhostRef = _parent->GetManagementObject()->getObjectId();
    exclusive = false;
    arguments = ::qpid::types::Variant::Map();
    altExchange = ::qmf2::ObjectId();
    consumerCount = 0;
    consumerCountHigh = 0;
    consumerCountLow  = 0;
    bindingCount = 0;
...
void Queue::debugStats (const std::string& comment)
{
...
void Queue::writeSchema (std::string& schema)
{
    ...
    buf.putShort       (47); // Inst Element Count
    ...
    ft.clear();
    ft[NAME] = "redirectPeer";
    ft[TYPE] = TYPE_SSTR;
    ft[DESC] = "Partner queue for redirected pair";
    buf.putMap(ft);

    ft.clear();
    ft[NAME] = "redirectSource";
    ft[TYPE] = TYPE_BOOL;
    ft[DESC] = "This queue is the redirect source";
    buf.putMap(ft);

    ft.clear();
    ft[NAME] = "creator";
    ft[TYPE] = TYPE_SSTR;
    ft[DESC] = "userId of creator of the queue";
    buf.putMap(ft);
...
uint32_t Queue::writePropertiesSize() const
{
    ...
    size += 1;  // autoDelete
    size += 1;  // exclusive
    size += :: qpid::broker::amqp0_10::MapCodec::encodedSize(arguments);  // arguments
...
void Queue::writeStatistics (std::string& _sBuf, bool skipHeaders)
{
    ...
    buf.putShortString(redirectPeer);
    buf.putOctet(redirectSource?1:0);
    buf.putShortString(creator);
...
void Queue::doMethod (string& methodName, const string& inStr, string& outStr, const string& userId)
{
    ...
    if (methodName == "purge") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_PURGE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_PURGE, ioArgs, text);
        else
    ...
    if (methodName == "reroute") {
        ...
        inBuf.getMap(ioArgs.i_filter);
        bool allow = manageable.AuthorizeMethod(METHOD_REROUTE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_REROUTE, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
                                              ...
                                              bool includeStatistics)
{
    ...
    _map["redirectPeer"] = ::qpid::types::Variant(redirectPeer);
    _map["redirectSource"] = ::qpid::types::Variant(redirectSource);
    _map["creator"] = ::qpid::types::Variant(creator);
