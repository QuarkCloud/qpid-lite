...
uint8_t Broker::md5Sum[MD5_LEN]   =
    {0x62,0x51,0xf,0xab,0x6e,0xc5,0x10,0x11,0x15,0xd1,0x3b,0xa0,0xa0,0xd5,0xa7,0xc1};

Broker::Broker (ManagementAgent*, Manageable* _core, ::qmf2::Manageable* _parent, const std::string& _name) :
...
void Broker::debugStats (const std::string& comment)
{
...
void Broker::writeSchema (std::string& schema)
{
    ...
    ft.clear();
    ft[NAME] =  "getLogHiresTimestamp";
    ft[ARGCOUNT] = 1;
    ft[DESC] = "Get the high resolution timestamp in logs";
    buf.putMap(ft);
    ...
    ft[TYPE] = TYPE_BOOL;
    ft[DIR] = "O";
    ft[DESC] = "True if high resolution timestamp in logs is enabled.";
    buf.putMap(ft);

    ft.clear();
    ft[NAME] =  "setLogHiresTimestamp";
    ft[ARGCOUNT] = 1;
    ft[DESC] = "Set the high resolution timestamp in logs";
    buf.putMap(ft);
    ...
    ft[TYPE] = TYPE_BOOL;
    ft[DIR] = "I";
    ft[DESC] = "True to enable enable high resolution timestamp in logs.";
    buf.putMap(ft);

    ft.clear();
    ft[NAME] =  "queueRedirect";
    ft[ARGCOUNT] = 2;
    ft[DESC] = "Enable/disable delivery redirect for indicated queues";
    buf.putMap(ft);
    ...
    ft[TYPE] = TYPE_SSTR;
    ft[DIR] = "I";
    ft[DESC] = "Redirect target queue. Blank disables redirect.";
    buf.putMap(ft);

    ft.clear();
    ft[NAME] =  "shutdown";
    ft[ARGCOUNT] = 0;
    ft[DESC] = "Shutdown the broker";
    buf.putMap(ft);
...
void Broker::readProperties (const std::string& _sBuf)
{
    ...
    ::qmf2::Buffer buf(_tmpBuf, _sBuf.length());
...
void Broker::doMethod (string& methodName, const string& inStr, string& outStr, const string& userId)
{
    ...
    const int _bufSize=65536;
    char _msgChars[_bufSize];
    ::qmf2::Buffer outBuf(_msgChars, _bufSize);


    char *_tmpBuf = new char[inStr.length()];
    memcpy(_tmpBuf, inStr.data(), inStr.length());
    ::qmf2::Buffer inBuf(_tmpBuf, inStr.length());
    ...
    if (methodName == "echo") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_ECHO, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_ECHO, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "connect") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_CONNECT, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_CONNECT, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "queueMoveMessages") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_QUEUEMOVEMESSAGES, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_QUEUEMOVEMESSAGES, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "setLogLevel") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_SETLOGLEVEL, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_SETLOGLEVEL, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "getLogLevel") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_GETLOGLEVEL, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_GETLOGLEVEL, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "getTimestampConfig") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_GETTIMESTAMPCONFIG, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_GETTIMESTAMPCONFIG, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "setTimestampConfig") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_SETTIMESTAMPCONFIG, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_SETTIMESTAMPCONFIG, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "create") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_CREATE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_CREATE, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "delete") {
        ...
        bool allow = manageable.AuthorizeMethod(METHOD_DELETE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_DELETE, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "query") {
        ...
        inBuf.getShortString(ioArgs.i_type);
        inBuf.getShortString(ioArgs.i_name);
        bool allow = manageable.AuthorizeMethod(METHOD_QUERY, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_QUERY, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "getLogHiresTimestamp") {
        _matched = true;
        ArgsBrokerGetLogHiresTimestamp ioArgs;
        bool allow = manageable.AuthorizeMethod(METHOD_GETLOGHIRESTIMESTAMP, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_GETLOGHIRESTIMESTAMP, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
        outBuf.putOctet(ioArgs.o_logHires?1:0);
    }

    if (methodName == "setLogHiresTimestamp") {
        _matched = true;
        ArgsBrokerSetLogHiresTimestamp ioArgs;
        ioArgs.i_logHires = inBuf.getOctet()==1;
        bool allow = manageable.AuthorizeMethod(METHOD_SETLOGHIRESTIMESTAMP, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_SETLOGHIRESTIMESTAMP, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "queueRedirect") {
        _matched = true;
        ArgsBrokerQueueRedirect ioArgs;
        inBuf.getShortString(ioArgs.i_sourceQueue);
        inBuf.getShortString(ioArgs.i_targetQueue);
        bool allow = manageable.AuthorizeMethod(METHOD_QUEUEREDIRECT, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_QUEUEREDIRECT, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
    ...
    if (methodName == "shutdown") {
        _matched = true;
        ::qmf2::ArgsNone ioArgs;
        bool allow = manageable.AuthorizeMethod(METHOD_SHUTDOWN, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_SHUTDOWN, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
...
void Broker::doMethod (string& methodName, const ::qpid::types::Variant::Map& inMap, ::qpid::types::Variant::Map& outMap, const string& userId)
{
    ...
    if (methodName == "echo") {
        ...
        outMap["_status_text"] = ::qmf2::Manageable::StatusText(status, text);
    ...
    if (methodName == "getLogHiresTimestamp") {
        ...
        outMap["_status_code"] = (uint32_t) status;
        outMap["_status_text"] = ::qmf2::Manageable::StatusText(status, text);
        outMap["logHires"] = ::qpid::types::Variant(ioArgs.o_logHires);
        return;
    }
    ...
    if (methodName == "setLogHiresTimestamp") {
        ArgsBrokerSetLogHiresTimestamp ioArgs;
        ::qpid::types::Variant::Map::const_iterator _i;
        if ((_i = inMap.find("logHires")) != inMap.end()) {
            ioArgs.i_logHires = _i->second;
        } else {
            ioArgs.i_logHires = false;
        }
        bool allow = manageable.AuthorizeMethod(METHOD_SETLOGHIRESTIMESTAMP, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_SETLOGHIRESTIMESTAMP, ioArgs, text);
    ...
    if (methodName == "queueRedirect") {
        ArgsBrokerQueueRedirect ioArgs;
        ::qpid::types::Variant::Map::const_iterator _i;
        if ((_i = inMap.find("sourceQueue")) != inMap.end()) {
            ioArgs.i_sourceQueue = (_i->second).getString();
        } else {
            ioArgs.i_sourceQueue = "";
        }
        if ((_i = inMap.find("targetQueue")) != inMap.end()) {
            ioArgs.i_targetQueue = (_i->second).getString();
        } else {
            ioArgs.i_targetQueue = "";
        }
        bool allow = manageable.AuthorizeMethod(METHOD_QUEUEREDIRECT, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_QUEUEREDIRECT, ioArgs, text);
