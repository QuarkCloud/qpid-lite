...
namespace qpid {
	namespace broker {
		namespace manage {
			...
			void EventBrokerLinkUp::writeSchema(std::string& schema)
			{
			...
			void EventBrokerLinkUp::encode(std::string& _sBuf) const
			{
				const int _bufSize = 65536;
				char _msgChars[_bufSize];
				::qmf2::Buffer buf(_msgChars, _bufSize);
			...
			void EventBrokerLinkUp::mapEncode(::qpid::types::Variant::Map& map) const
			{
