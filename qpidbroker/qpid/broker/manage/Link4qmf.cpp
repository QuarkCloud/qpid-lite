...
#include "qpid/broker/manage/ArgsLinkBridge.h"
...
uint8_t Link::md5Sum[MD5_LEN]   =
    {0x56,0x73,0x82,0x5,0xac,0x48,0x99,0x8d,0xc0,0x53,0xa8,0x71,0xfb,0x27,0x64,0xfd};

Link::Link (ManagementAgent*, Manageable* _core, ::qmf2::Manageable* _parent, const std::string& _name, bool _durable) :
...
void Link::debugStats (const std::string& comment)
{
...
void Link::writeSchema (std::string& schema)
{
    ...
    ft[TYPE] = TYPE_U32;
    ft[DIR] = "I";
    ft[DEFAULT] = "0xFFFFFFFF";
    ft[DESC] = "granted to peer, 0 = infinite";
    buf.putMap(ft);
...
void Link::doMethod (string& methodName, const string& inStr, string& outStr, const string& userId)
{
    ...
    if (methodName == "bridge") {
        ...
        ioArgs.i_credit = inBuf.getLong();
        bool allow = manageable.AuthorizeMethod(METHOD_BRIDGE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_BRIDGE, ioArgs, text);
        else
            status = Manageable::STATUS_FORBIDDEN;
        outBuf.putLong        (status);
        outBuf.putMediumString(::qmf2::Manageable::StatusText (status, text));
...
void Link::doMethod (string& methodName, const ::qpid::types::Variant::Map& inMap, ::qpid::types::Variant::Map& outMap, const string& userId)
{
    ...
    if (methodName == "bridge") {
        ...
        if ((_i = inMap.find("credit")) != inMap.end()) {
            ioArgs.i_credit = _i->second;
        } else {
            ioArgs.i_credit = 0xFFFFFFFF;
        }
        bool allow = manageable.AuthorizeMethod(METHOD_BRIDGE, ioArgs, userId);
        if (allow)
            status = manageable.ManagementMethod (METHOD_BRIDGE, ioArgs, text);
