...
void Exchange::doRoute(Deliverable& msg, ConstBindingList b)
{
    ...
    if (b.get()) {
        ...
        for(std::vector<Binding::shared_ptr>::const_iterator i = b->begin(); i != b->end(); i++, count++) {
            try {
                ...
                error.store(ExInfo::OTHER, qpid::sys::ExceptionHolder(new qpid::sys::Exception(e.what())), (*i)->queue);
            }
        }
        error.raise();
    }
    ...
    if (mgmtExchange != 0)
    {
        qpid::broker::manage::Exchange::PerThreadStats *eStats = mgmtExchange->getStatistics();
    ...
    sequenceNo(0), ive(false), broker(b), destroyed(false)
{
    if (parent != 0 && broker != 0)
    {
        ...
        if (agent != 0)
        {
            ...
            if (broker)
                brokerMgmtObject = boost::dynamic_pointer_cast<qpid::broker::manage::Broker>(broker->GetManagementObject());
        }
    }
}
...
void Exchange::registerDynamicBridge(DynamicBridge* db)
{
    if (!supportsDynamicBinding())
        throw qpid::sys::Exception("Exchange type does not support dynamic binding");
    ...
    if (parent != 0)
    {
        ...
        if (broker != 0) {
            ...
            if (agent != 0) {
                _qmf::Queue::shared_ptr mo = boost::dynamic_pointer_cast<_qmf::Queue>(queue->GetManagementObject());
                if (mo != 0) {
                    ::qmf2::ObjectId queueId = mo->getObjectId();
...
//    qpid::broker::amqp0_10::MessageTransfer::setExchange(msg, getName());
//}
