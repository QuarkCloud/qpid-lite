...
#include "qpid/framing/UrlArray.h"
...
public:
    ...
    void route(broker::Deliverable& /*msg*/)
    {
        ...
        if (headers && headers->getArray(FAILOVER_HEADER_KEY, addresses)) {
            // convert the Array of addresses to a single sys::Url container for used with setUrl():
            std::vector<sys::Url> urlVec;
            sys::Url urls;
...
void Link::established(qpid::broker::amqp0_10::Connection* c)
{
    stringstream addr;
    addr << host << ":" << port;
...
void Link::setUrl(const sys::Url& u) {
...
void Link::opened()
{
    ...
    if (url.empty()) {
        const std::vector<sys::Url>& known = connection->getKnownHosts();
...
void Link::closed(int, std::string text)
{
    ...
    bool isClosing = false;
    {
        ...
        mgmtObject->set_connectionRef(qmf2::ObjectId());
...
void Link::reconnectLH(const sys::Address& a)
{
    host = a.host;
    port = a.port;
...
bool Link::tryFailoverLH() {
    ...
    sys::Address next = url[reconnectNext++];
...
framing::ChannelId Link::nextChannel()
{
    ...
    throw qpid::sys::Exception(sys::Msg() << "Link " << name << " channel pool is empty");
}
...
bool Link::getRemoteAddress(qpid::sys::Address& addr) const
{
