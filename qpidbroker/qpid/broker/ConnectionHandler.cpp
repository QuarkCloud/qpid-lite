...
#include "qpid/base/SaslFactory.h"
...
namespace qpid {
	namespace broker {
		...
		ConnectionHandler::ConnectionHandler(qpid::broker::amqp0_10::Connection& connection, bool isClient) :
			handler(new Handler(connection, isClient)) {}

		ConnectionHandler::Handler::Handler(qpid::broker::amqp0_10::Connection& c, bool isClient) :
			proxy(c.getOutput()),
		...
		void ConnectionHandler::Handler::startOk(const ConnectionStartOkBody& body)
		{
			...
			qpid::broker::manage::Connection::shared_ptr mgmtObject = connection.getMgmtObject();
			types::Variant::Map properties;
			qpid::broker::amqp0_10::translate(clientProperties, properties);
			...
			try {
				...
				::qpid::broker::ManagementAgent* agent = connection.getAgent();
				bool logEnabled;
			...
			const framing::Array& /*capabilities*/, bool /*insist*/)
		{
			...
			std::vector<sys::Url> urls = connection.getBroker().getKnownBrokers();
			framing::Array array(0x95); // str16 array
			for (std::vector<sys::Url>::iterator i = urls.begin(); i < urls.end(); ++i)
			...
			const framing::Array& /*locales*/)
		{
			...
			if (connection.getBroker().isAuthenticating()) {
				sasl = base::SaslFactory::getInstance().create(username,
					password,
					service,
					host,
