...
void ThresholdAlerts::enqueued(const Message& m)
{
    ...
    if (sizeGoingUp && sizeThreshold && size >= sizeThreshold) {
        sizeGoingUp = false;
        agent.raiseEvent(qpid::broker::manage::EventQueueThresholdCrossedUpward(name, count, size));
        if (backwardCompat)
            agent.raiseEvent(qpid::broker::manage::EventQueueThresholdExceeded(name, count, size));
    }
...
void ThresholdAlerts::dequeued(const Message& m)
{
    ...
    if (!sizeGoingUp && sizeThreshold && size <= sizeThresholdDown) {
        sizeGoingUp = true;
        agent.raiseEvent(qpid::broker::manage::EventQueueThresholdCrossedDownward(name, count, size));
    }
...
void ThresholdAlerts::observe(Queue& queue, ::qpid::broker::ManagementAgent& agent,
                              const uint64_t ctu,
                              const uint64_t _ctd,
                              const uint64_t stu,
                              const uint64_t _std)
                              ...
                              const QueueSettings& settings, uint16_t limitRatio)
{
    ...
    uint32_t sizeThreshold = settings.alertThreshold.hasSize() ? 
		(uint32_t)settings.alertThreshold.getSize() : (uint32_t)(settings.maxDepth.getSize()*limitRatio/100);
    uint32_t countThresholdDown = settings.alertThresholdDown.hasCount() ? 
		(uint32_t)settings.alertThresholdDown.getCount() : 0;
    uint32_t sizeThresholdDown = settings.alertThresholdDown.hasSize() ? 
		(uint32_t)settings.alertThresholdDown.getSize() : 0;
