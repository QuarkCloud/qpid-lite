...
{
    if (directory.empty()) {
        throw qpid::sys::Exception(QPID_MSG("Cannot create paged queue: No paged queue directory specified"));
    }
...
void PagedQueue::publish(const Message& added)
{
    ...
    if (!newPage(added.getSequence()).add(added)) {
        ...
        throw qpid::sys::Exception(QPID_MSG("Could not add message to paged queue " << name));
    }
}
