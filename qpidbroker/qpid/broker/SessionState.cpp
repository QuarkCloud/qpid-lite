...
SessionState::SessionState(
    Broker& b, SessionHandler& h, const base::SessionId& id,
    const SessionState::Configuration& config)
    : base::SessionState(id, config),
      broker(b), handler(&h),
      semanticState(*this),
...
amqp0_10::Connection& SessionState::getConnection() {
    assert(isAttached());
...
void SessionState::handleContent(AMQFrame& frame)
{
    ...
    intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> msg(msgBuilder.getMessage());
...
DeliveryId SessionState::deliver(const qpid::broker::amqp0_10::MessageTransfer& message,
                                 ...
                                 const qpid::types::Variant::Map& annotations, bool sync)
{
    ...
    assert(senderGetCommandPoint() == base::SessionPoint(commandId+1, 0)); // Delivery has moved sendPoint.
    if (sync) {
...
void SessionState::senderCompleted(const SequenceSet& commands) {
    qpid::base::SessionState::senderCompleted(commands);
...
void SessionState::AsyncCommandCompleter::addPendingMessage(boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> msg)
{
    qpid::sys::ScopedLock<qpid::sys::Mutex> l(completerLock);
    std::pair<SequenceNumber, boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> > item(msg->getCommandId(), msg);
...
{
    std::map<SequenceNumber, boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> > copy;
    {
        qpid::sys::ScopedLock<qpid::sys::Mutex> l(completerLock);
    ...
    for (std::map<SequenceNumber, boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> >::iterator i = copy.begin();
         i != copy.end(); ++i) {
        i->second->flush();
    }
}
