...
void PersistableObject::encode(framing::Buffer& buffer) const
{
    ...
    buffer.putMediumString(name);
     qpid::broker::amqp0_10::encode(properties,  qpid::broker::amqp0_10::encodedSize(properties), buffer);
}
uint32_t PersistableObject::encodedSize() const
{
    return type.size()+1 + name.size()+2 +  qpid::broker::amqp0_10::encodedSize(properties);
}
void PersistableObject::decode(framing::Buffer& buffer)
{
    ...
    buffer.get(ft);
     qpid::broker::amqp0_10::translate(ft, properties);
}
