...
SessionHandler::SessionHandler(broker::amqp0_10::Connection& c, ChannelId ch)
    :  qpid::base::amqp0_10::SessionHandler(&c.getOutput(), ch),
      connection(c),
      proxy(out),
...
amqp0_10::Connection& SessionHandler::getConnection() { return connection; }

const amqp0_10::Connection& SessionHandler::getConnection() const { return connection; }

void SessionHandler::handleDetach() {
     qpid::base::amqp0_10::SessionHandler::handleDetach();
...
void SessionHandler::setState(const std::string& name, bool force) {
    assert(!session.get());
    base::SessionId id(connection.getUserId(), name);
...
qpid::base::SessionState* SessionHandler::getState() { return session.get(); }
...
void SessionHandler::attached(const std::string& name)
{
    if (session.get()) {
        session->addManagementObject(); // Delayed from attachAs()
         qpid::base::amqp0_10::SessionHandler::attached(name);
    } else {
		base::SessionId id(connection.getUserId(), name);
        SessionState::Configuration config = connection.getBroker().getSessionManager().getSessionConfig();
        session.reset(new SessionState(connection.getBroker(), *this, id, config));
