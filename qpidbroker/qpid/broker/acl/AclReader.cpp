				...
				for (int cnt = rules.size(); cnt; cnt--) {
					...
					if (!foundmode && (*i)->actionAll && (*i)->names.size() == 1
						...
						if (pName != (*i)->props.end()) {
							throw qpid::sys::Exception(QPID_MSG("ACL: CREATE CONNECTION rule " << cnt << " must not have a 'name' property"));
						}
						propMap::const_iterator pHost = (*i)->props.find(SPECPROP_HOST);
						if (pHost == (*i)->props.end()) {
							throw qpid::sys::Exception(QPID_MSG("ACL: CREATE CONNECTION rule " << cnt << " has no 'host' property"));
						}
						...
						if (allUsers) {
							if (allHosts) {
								...
								if (foundConnectionMode) {
									throw qpid::sys::Exception(QPID_MSG("ACL: only one CREATE CONNECTION rule for user=all and host=all allowed"));
								}
						...
						for (int acnt = ((*i)->actionAll ? 0 : (*i)->action);
							...
							if (d->actionList[acnt] == NULL) {
								d->actionList[acnt] =
									new AclData::aclAction[qpid::broker::acl::OBJECTSIZE];
								for (int j = 0; j < qpid::broker::acl::OBJECTSIZE; j++)
