...
namespace qpid {
	namespace broker {
		namespace acl {
			 ...
			 */
			struct AclOptions : public sys::Options {
				AclValues& values;

				AclOptions(AclValues& v) : Options("ACL Options"), values(v) {
					values.aclMaxConnectTotal = 500;
					addOptions()
						("acl-file", sys::optValue(values.aclFile, "FILE"), "The policy file to load from, loaded from data dir")
						("connection-limit-per-user", sys::optValue(values.aclMaxConnectPerUser, "N"), "The maximum number of connections allowed per user. 0 implies no limit.")
						("max-connections", sys::optValue(values.aclMaxConnectTotal, "N"), "The maximum combined number of connections allowed. 0 implies no limit.")
						("connection-limit-per-ip", sys::optValue(values.aclMaxConnectPerIp, "N"), "The maximum number of connections allowed per host IP address. 0 implies no limit.")
						("max-queues-per-user", sys::optValue(values.aclMaxQueuesPerUser, "N"), "The maximum number of queues allowed per user. 0 implies no limit.")
						;
				}
			};

			struct AclPlugin : public base::Plugin {
				...
				sys::Options* getOptions() { return &options; }

				void init(broker::Broker& b) {
					if (acl) throw qpid::sys::Exception("ACL plugin cannot be initialized twice in one process.");
