...
#include "qpid/broker/acl/AclLexer.h"
...
#define RP registerProperties
			...
			void AclValidator::validate(boost::shared_ptr<AclData> d) {
				...
				for (unsigned int cnt = 0; cnt < qpid::broker::acl::ACTIONSIZE; cnt++) {

					if (d->actionList[cnt]) {

						for (unsigned int cnt1 = 0; cnt1 < qpid::broker::acl::OBJECTSIZE; cnt1++) {
			...
			void AclValidator::validateRuleSet(std::pair<const std::string, qpid::broker::acl::AclData::ruleSet>& rules) {
				std::for_each(rules.second.begin(),
					rules.second.end(),
					boost::bind(&AclValidator::validateRule, this, _1));
			}

			void AclValidator::validateRule(qpid::broker::acl::AclData::Rule& rule) {
				std::for_each(rule.props.begin(),
					rule.props.end(),
					boost::bind(&AclValidator::validateProperty, this, _1));
			}

			void AclValidator::validateProperty(std::pair<const qpid::broker::acl::SpecProperty, std::string>& prop) {
				...
				if (itr != validators.end()) {
					...
					if (!itr->second->validate(prop.second)) {
						...
						throw qpid::sys::Exception(prop.second + " is not a valid value for '" +
							AclHelper::getPropertyStr(prop.first) + "', " +
							itr->second->allowedValues());
					}
				}
			 ...
			 */
			bool AclValidator::validateAllowedProperties(qpid::broker::acl::Action action,
			 ...
			 */
			void AclValidator::findPossibleLookupMatch(qpid::broker::acl::Action action,
				qpid::broker::acl::ObjectType object,
				const AclData::specPropertyMap& props,
			...
			void AclValidator::registerProperties(
				...
				std::vector<std::string> props = sys::split(properties, " ");
				...
				for (size_t i = 0; i < props.size(); i++) {
					...
					case PROP_MAXFILECOUNT:
						...
						break;
					default:
						throw qpid::sys::Exception("acl::RegisterProperties no case for property: " +
