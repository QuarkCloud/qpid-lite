...
#include "qpid/broker/acl/Acl.h"
#include "qpid/broker/acl/AclConnectionCounter.h"
#include "qpid/broker/acl/AclResourceCounter.h"
#include "qpid/broker/acl/AclData.h"
#include "qpid/broker/acl/AclValidator.h"
#include "qpid/sys/Mutex.h"

#include "qpid/broker/Broker.h"
#include "qpid/broker/Connection.h"
#include "qpid/base/Plugin.h"
		...
		namespace acl {
				...
				resourceCounter(new ResourceCounter(*this, aclValues.aclMaxQueuesPerUser)), userRules(false)
			{
				...
				if (aclValues.aclMaxConnectPerUser > AclData::getConnectMaxSpec())
					throw qpid::sys::Exception("--connection-limit-per-user switch cannot be larger than " + AclData::getMaxConnectSpecStr());
				if (aclValues.aclMaxConnectPerIp > AclData::getConnectMaxSpec())
					throw qpid::sys::Exception("--connection-limit-per-ip switch cannot be larger than " + AclData::getMaxConnectSpecStr());
				if (aclValues.aclMaxConnectTotal > AclData::getConnectMaxSpec())
					throw qpid::sys::Exception("--max-connections switch cannot be larger than " + AclData::getMaxConnectSpecStr());
				if (aclValues.aclMaxQueuesPerUser > AclData::getConnectMaxSpec())
					throw qpid::sys::Exception("--max-queues-per-user switch cannot be larger than " + AclData::getMaxQueueSpecStr());
				...
				if (!aclValues.aclFile.empty()) {
					...
					if (!readAclFile(errorString)) {
						...
						throw qpid::sys::Exception("Could not read ACL file " + errorString);
					}
				}
				else {
					loadEmptyAclRuleset();
			...
			//
			Manageable::status_t Acl::lookup(qmf2::Args& args, std::string& text)
			{
			...
			//
			Manageable::status_t Acl::lookupPublish(qmf2::Args& args, std::string& /*text*/)
			{
