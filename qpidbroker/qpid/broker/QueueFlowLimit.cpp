...
void QueueFlowLimit::dequeued(const Message& msg)
{
    ...
    if (count > 0) {
        ...
        throw qpid::sys::Exception(QPID_MSG("Flow limit count underflow on dequeue. Queue=" << queueName));
    }
    ...
    if (_size <= size) {
        ...
        throw qpid::sys::Exception(QPID_MSG("Flow limit size underflow on dequeue. Queue=" << queueName));
    }
