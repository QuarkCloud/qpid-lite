...
#include "qpid/base/amqp/MessageId.h"
...
base::amqp::MessageId MessageTransfer::getMessageId() const
{
    const qpid::framing::MessageProperties* mp = getProperties<qpid::framing::MessageProperties>();

	base::amqp::MessageId r;
    if (mp->hasMessageId()) {
        r.set(base::amqp::CharSequence::create(mp->getMessageId().data(),16), types::VAR_UUID);
    }
    return r;
}

base::amqp::MessageId MessageTransfer::getCorrelationId() const
{
    const qpid::framing::MessageProperties* mp = getProperties<qpid::framing::MessageProperties>();

	base::amqp::MessageId r;
    if (mp->hasCorrelationId()) {
        r.set(base::amqp::CharSequence::create(mp->getCorrelationId()), types::VAR_STRING);
    }
    return r;
}
        ...
        if (redelivered || ttl || annotations.size()) {
            ...
            if (annotations.size()) {
                ...
                for (qpid::types::Variant::Map::const_iterator i = annotations.begin();
                     i != annotations.end(); ++i) {
                    props->getApplicationHeaders().set(i->first, qpid::broker::amqp0_10::translate(i->second));
                }
            }
            if (redelivered || ttl) {
...
{
    return  (uint32_t)frames.getContentSize();
}
...
void MessageTransfer::processProperties(qpid::base::amqp::MapHandler& handler) const
{
    const qpid::framing::MessageProperties* mp = getProperties<qpid::framing::MessageProperties>();
    if (mp && mp->hasApplicationHeaders()) {
        ...
        for (FieldTable::const_iterator i = ft.begin(); i != ft.end(); ++i) {
            ...
            qpid::broker::amqp0_10::translate(i->second, v);
            qpid::base::amqp::CharSequence key = {i->first.data(), i->first.size()};
            switch (v.getType()) {
            ...
            case qpid::types::VAR_STRING: {
                std::string s(v);
                qpid::base::amqp::CharSequence value = {s.data(), s.size()};
                qpid::base::amqp::CharSequence encoding = {0, 0};
...
boost::intrusive_ptr<PersistableMessage> MessageTransfer::merge(const std::map<std::string, qpid::types::Variant>& annotations) const
{
    ...
    for (qpid::types::Variant::Map::const_iterator i = annotations.begin(); i != annotations.end(); ++i) {
        mp->getApplicationHeaders().set(i->first, qpid::broker::amqp0_10::translate(i->second));
    }
    return clone;
}
