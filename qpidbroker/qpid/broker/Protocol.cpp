...
#include "qpid/base/amqp0_10/Connection.h"
...
namespace qpid {
	namespace broker {
		...
		typedef std::auto_ptr<qpid::base::amqp0_10::Connection> CodecPtr;
		typedef std::auto_ptr<SecureConnection> SecureConnectionPtr;
		typedef std::auto_ptr<qpid::broker::amqp0_10::Connection> ConnectionPtr;
		typedef std::auto_ptr<base::ConnectionInputHandler> InputPtr;
				...
				const qpid::sys::SecuritySettings& external, bool brokerActsAsClient)
		{
			...
			CodecPtr c(new  qpid::base::amqp0_10::Connection(out, id, brokerActsAsClient));
			ConnectionPtr i(new broker::amqp0_10::Connection(c.get(), *broker, id, external, brokerActsAsClient));
		...
		boost::intrusive_ptr<const qpid::broker::amqp0_10::MessageTransfer> ProtocolRegistry::translate(const Message& m)
		{
			boost::intrusive_ptr<const qpid::broker::amqp0_10::MessageTransfer> transfer;
			const qpid::broker::amqp0_10::MessageTransfer* ptr = dynamic_cast<const qpid::broker::amqp0_10::MessageTransfer*>(&m.getEncoding());
			if (ptr) transfer = boost::intrusive_ptr<const qpid::broker::amqp0_10::MessageTransfer>(ptr);
			for (Protocols::const_iterator i = protocols.begin(); !transfer && i != protocols.end(); ++i) {
				transfer = i->second->translate(m);
			}
			if (!transfer) throw new qpid::sys::Exception("Could not convert message into 0-10");
			return transfer;
		}
		boost::shared_ptr<RecoverableMessage> ProtocolRegistry::recover(qpid::framing::Buffer& b)
		{
			...
			boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> transfer(new qpid::broker::amqp0_10::MessageTransfer());
