...
#include "qpid/broker/amqp0_10/MessageTransfer.h"
...
using qpid::broker::amqp0_10::MapCodec;
using qpid::broker::amqp0_10::ListCodec;
...
struct ScopedManagementContext
{
    ...
    qmf2::ObjectId getObjectId() const
    {
        return context ? context->getObjectId() : qmf2::ObjectId();
    }
    ...
    qmf1Support(qmfV1), qmf2Support(qmfV2), maxReplyObjs(100)
{
    ...
    memstat = _qmf::Memory::shared_ptr(new ::qpid::broker::manage::Memory(this, 0, "amqp-broker"));
                                ...
                                qpid::broker::Broker* _broker, int _threads)
{
    ...
    qmf2::ManagementObject::maxThreads = threadPoolSize;
    sendQueue.reset(
...
void ManagementAgent::setName(const string& vendor, const string& product, const string& instance)
{
    if (vendor.find(':') != vendor.npos) {
        throw qpid::sys::Exception("vendor string cannot contain a ':' character.");
    }
    if (product.find(':') != product.npos) {
        throw qpid::sys::Exception("product string cannot contain a ':' character.");
    }
    ...
    if (instance.empty()) {
        if (uuid.isNull())
        {
            throw qpid::sys::Exception("ManagementAgent::configure() must be called if default name is used.");
        }
        inst = uuid.str();
    } else
        inst = instance;
...
void ManagementAgent::registerClass (const string&  packageName,
                                     ...
                                     uint8_t* md5Sum,
                                     qmf2::ManagementObject::writeSchemaCall_t schemaCall)
{
    sys::Mutex::ScopedLock lock(userLock);
    PackageMap::iterator pIter = findOrAddPackageLH(packageName);
    addClassLH(qmf2::ManagementItem::CLASS_KIND_TABLE, pIter, className, md5Sum, schemaCall);
}
	...
	qmf2::ManagementObject::writeSchemaCall_t schemaCall)
{
    sys::Mutex::ScopedLock lock(userLock);
    PackageMap::iterator pIter = findOrAddPackageLH(packageName);
    addClassLH(qmf2::ManagementItem::CLASS_KIND_EVENT, pIter, eventName, md5Sum, schemaCall);
}

// Deprecated:  V1 objects
qmf2::ObjectId ManagementAgent::addObject(qmf2::ManagementObject::shared_ptr object, uint64_t persistId, bool persistent)
{
    uint16_t sequence;
    uint64_t objectNum;
	...
	qmf2::ObjectId objId(0 /*flags*/, sequence, brokerBank, objectNum);
...
qmf2::ObjectId ManagementAgent::addObject(qmf2::ManagementObject::shared_ptr object,
                                    const string& key,
                                    bool persistent)
{
    uint16_t sequence;
	...
	qmf2::ObjectId objId(0 /*flags*/, sequence, brokerBank);
    if (key.empty()) {
...
void ManagementAgent::raiseEvent(const qmf2::ManagementEvent& event, severity_t severity)
{
                                 ...
                                 const string&  routingKey)
{
    ...
    intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> transfer(new qpid::broker::amqp0_10::MessageTransfer());
...
void ManagementAgent::moveNewObjects()
{
    ...
    while (!newManagementObjects.empty()) {
		qmf2::ManagementObject::shared_ptr object = newManagementObjects.back();
        ...
        if (object->isDeleted()) {
			...
			qmf2::ObjectId oid = object->getObjectId();
			qmf2::ManagementObjectMap::iterator destIter = managementObjects.find(oid);
            if (destIter != managementObjects.end()) {
				...
				qmf2::ManagementObject::shared_ptr oldObj = destIter->second;
                if (!oldObj->isDeleted()) {
...
#define HEADROOM  4096
    ...
    if (publish) {
		...
		qpid::broker::manage::MemStat::loadMemInfo(memstat.get());
    }

    //
    //  Use a copy of the management object map to avoid holding the objectLock
    //
	qmf2::ManagementObjectVector localManagementObjects;
    {
        sys::Mutex::ScopedLock objLock(objectLock);
        std::transform(managementObjects.begin(), managementObjects.end(),
                       std::back_inserter(localManagementObjects),
                       boost::bind(&qmf2::ManagementObjectMap::value_type::second, _1));
    }

    //
    //  Clear the been-here flag on all objects in the map.
    //
    for (qmf2::ManagementObjectVector::iterator iter = localManagementObjects.begin();
         iter != localManagementObjects.end();
         iter++) {
		qmf2::ManagementObject::shared_ptr object = *iter;
        object->setFlags(0);
        if (clientWasAdded) {
    ...
    while (publish) {
		...
		qmf2::ManagementObjectVector::iterator baseIter;
        std::string packageName;
        std::string className;
        ...
        for (baseIter = localManagementObjects.begin();
             ...
             baseIter++) {
			qmf2::ManagementObject::shared_ptr baseObject = *baseIter;
            //
        ...
        for (qmf2::ManagementObjectVector::iterator iter = baseIter;
             iter != localManagementObjects.end();
             iter++) {
            msgBuffer.makeAvailable(HEADROOM); // Make sure there's buffer space
			qmf2::ManagementObject::shared_ptr baseObject = *baseIter;
...
void ManagementAgent::deleteObjectNow(const ::qmf2::ObjectId& oid)
{
	qmf2::ManagementObject::shared_ptr object;
    {
        sys::Mutex::ScopedLock lock(objectLock);
		qmf2::ManagementObjectMap::iterator iter = managementObjects.find(oid);
                                    ...
                                    const string& text, uint32_t code, bool viaLocal)
{
    ...
    QPID_LOG(debug, "SENT qpid::sys::Exception code=" << code <<" text=" << text);
}
...
void ManagementAgent::handleMethodRequest(Buffer& inBuffer, const string& replyToKey, uint32_t sequence, const string& userId)
{
    ...
    if (disallowAllV1Methods) {
        outBuffer.putLong(qmf2::Manageable::STATUS_FORBIDDEN);
    ...
    if (acl != 0) {
        ...
        if (!acl->authorise(userId, acl::ACT_ACCESS, acl::OBJ_METHOD, methodName, &params)) {
            outBuffer.putLong(qmf2::Manageable::STATUS_FORBIDDEN);
            outBuffer.putMediumString(qmf2::Manageable::StatusText(qmf2::Manageable::STATUS_FORBIDDEN));
            sendBuffer(outBuffer, dExchange, replyToKey);
    ...
    {
        sys::Mutex::ScopedLock lock(objectLock);
		qmf2::ManagementObjectMap::iterator iter = numericFind(objId);
        if (iter != managementObjects.end())
            object = iter->second;
    }

    if (!object || object->isDeleted()) {
        outBuffer.putLong        (qmf2::Manageable::STATUS_UNKNOWN_OBJECT);
        outBuffer.putMediumString(qmf2::Manageable::StatusText (qmf2::Manageable::STATUS_UNKNOWN_OBJECT));
    } else {
        if ((object->getPackageName() != packageName) ||
            (object->getClassName()   != className)) {
            outBuffer.putLong        (qmf2::Manageable::STATUS_PARAMETER_INVALID);
            outBuffer.putMediumString(qmf2::Manageable::StatusText (qmf2::Manageable::STATUS_PARAMETER_INVALID));
        }
        else {
            ...
            try {
                ...
                outBuffer.putLong(qmf2::Manageable::STATUS_EXCEPTION);
                                           ...
                                           const string& cid, const string& userId, bool viaLocal)
{
    ...
    if ((oid = inMap.find("_object_id")) == inMap.end() ||
        ...
        sendException(rte, rtk, cid, qmf2::Manageable::StatusText(qmf2::Manageable::STATUS_PARAMETER_INVALID),
			qmf2::Manageable::STATUS_PARAMETER_INVALID, viaLocal);
        return;
    }
    ...
    try {
        ...
        objId = qmf2::ObjectId(oid->second.asMap());
        ...
        sendException(rte, rtk, cid, e.what(), qmf2::Manageable::STATUS_EXCEPTION, viaLocal);
        return;
    }
    ...
    {
        sys::Mutex::ScopedLock lock(objectLock);
		qmf2::ManagementObjectMap::iterator iter = managementObjects.find(objId);
        if (iter != managementObjects.end())
            object = iter->second;
    }
    ...
    if (i != disallowed.end()) {
        sendException(rte, rtk, cid, i->second, qmf2::Manageable::STATUS_FORBIDDEN, viaLocal);
        return;
    }
    ...
    if (acl != 0) {
        ...
        if (!acl->authorise(userId, acl::ACT_ACCESS, acl::OBJ_METHOD, methodName, &params)) {
            sendException(rte, rtk, cid, qmf2::Manageable::StatusText(qmf2::Manageable::STATUS_FORBIDDEN),
				qmf2::Manageable::STATUS_FORBIDDEN, viaLocal);
            return;
        }
    }
...
{
    list<qmf2::ObjectId> deleteList;

    for (RemoteAgentMap::const_iterator aIter = remoteAgents.begin(); aIter != remoteAgents.end(); aIter++) {
        bool found = false;

        for (qmf2::ManagementObjectMap::iterator iter = managementObjects.begin();
    ...
    for (list<qmf2::ObjectId>::const_iterator dIter = deleteList.begin(); dIter != deleteList.end(); dIter++)
        remoteAgents.erase(*dIter);
}

void ManagementAgent::handleAttachRequest (Buffer& inBuffer, const string& replyToKey, uint32_t sequence, 
	const qmf2::ObjectId& connectionRef)
{
    string   label;
...
void ManagementAgent::handleGetQuery(Buffer& inBuffer, const string& replyToKey, uint32_t sequence, const string& userId)
{
    ...
    if (value.get() == 0 || !value->convertsTo<string>()) {
		...
		qmf2::ObjectId selector(value->get<string>());

		qmf2::ManagementObject::shared_ptr object;
        {
            sys::Mutex::ScopedLock lock(objectLock);
			::qmf2::ManagementObjectMap::iterator iter = numericFind(selector);
            if (iter != managementObjects.end())
                object = iter->second;
        }
    ...
    std::list<qmf2::ManagementObject::shared_ptr> matches;
    ...
    // build up a set of all objects to be dumped
    {
        ...
        for (qmf2::ManagementObjectMap::iterator iter = managementObjects.begin();
             iter != managementObjects.end();
             iter++) {
			qmf2::ManagementObject::shared_ptr object = iter->second;
    ...
    ResizableBuffer outBuffer (qmfV1BufferSize);
    while (matches.size()) {
		qmf2::ManagementObject::shared_ptr object = matches.front();
        if (object->getConfigChanged() || object->getInstChanged())
            object->setUpdateTime();
...
void ManagementAgent::handleGetQuery(const string& body, const string& rte, const string& rtk, const string& cid, const std::string& userId, bool viaLocal)
{
    ...
    if (i != inMap.end() && i->second.getType() == qpid::types::VAR_MAP) {
        Variant::List list_;
		qmf2::ObjectId objId(i->second.asMap());
...
bool ManagementAgent::authorizeAgentMessage(Message& msg)
{
    ...
    boost::intrusive_ptr<const qpid::broker::amqp0_10::MessageTransfer> transfer = protocols->translate(msg);
    //
    ...
    if (methodReq) {
        ...
        if (p && p->hasReplyTo()) {
            ...
            if (mapMsg) {
                sendException(rte, rtk, cid, ::qmf2::Manageable::StatusText(::qmf2::Manageable::STATUS_FORBIDDEN),
					::qmf2::Manageable::STATUS_FORBIDDEN, false);
            } else {
...
{
    if      (kind == ::qmf2::ManagementItem::CLASS_KIND_TABLE)
        return validateTableSchema(inBuffer);
    else if (kind == ::qmf2::ManagementItem::CLASS_KIND_EVENT)
...
size_t ManagementAgent::validateEventSchema(Buffer& inBuffer)
{
    ...
    try {
        uint8_t kind = inBuffer.getOctet();
        if (kind != ::qmf2::ManagementItem::CLASS_KIND_EVENT)
            return 0;
...
::qmf2::ManagementObjectMap::iterator ManagementAgent::numericFind(const ::qmf2::ObjectId& oid)
{
	::qmf2::ManagementObjectMap::iterator iter = managementObjects.begin();
...
namespace {
bool isDeletedMap(const ::qmf2::ManagementObjectMap::value_type& value) {
    return value.second->isDeleted();
}

bool isDeletedVector(const ::qmf2::ManagementObjectVector::value_type& value) {
    return value->isDeleted();
}

string summarizeMap(const char* name, const ::qmf2::ManagementObjectMap& map) {
    ostringstream o;
...
string summarizeVector(const char* name, const ::qmf2::ManagementObjectVector& map) {
    ostringstream o;
...
string dumpMap(const ::qmf2::ManagementObjectMap& map) {
    ostringstream o;
    for (::qmf2::ManagementObjectMap::const_iterator i = map.begin(); i != map.end(); ++i) {
...
string dumpVector(const ::qmf2::ManagementObjectVector& map) {
    ostringstream o;
    for (::qmf2::ManagementObjectVector::const_iterator i = map.begin(); i != map.end(); ++i) {
...
{
    Variant::Map map;
    qpid::broker::amqp0_10::translate(from, map);
    return map;
}

// construct a DeletedObject from a management object.
ManagementAgent::DeletedObject::DeletedObject(::qmf2::ManagementObject::shared_ptr src, bool v1, bool v2)
...
bool ManagementAgent::moveDeletedObjects() {
    typedef vector<pair<::qmf2::ObjectId, ::qmf2::ManagementObject::shared_ptr> > DeleteList;
    ...
    for (DeleteList::reverse_iterator iter = deleteList.rbegin();
         ...
         iter++)
    {
		::qmf2::ManagementObject::shared_ptr delObj = iter->second;
