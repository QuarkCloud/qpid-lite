...
#include "qpid/broker/manage/ArgsBrokerConnect.h"
#include "qpid/broker/manage/ArgsBrokerCreate.h"
#include "qpid/broker/manage/ArgsBrokerDelete.h"
#include "qpid/broker/manage/ArgsBrokerQuery.h"
#include "qpid/broker/manage/ArgsBrokerEcho.h"
#include "qpid/broker/manage/ArgsBrokerGetLogLevel.h"
#include "qpid/broker/manage/ArgsBrokerQueueMoveMessages.h"
#include "qpid/broker/manage/ArgsBrokerSetLogLevel.h"
#include "qpid/broker/manage/ArgsBrokerGetLogHiresTimestamp.h"
#include "qpid/broker/manage/ArgsBrokerSetLogHiresTimestamp.h"
#include "qpid/broker/manage/ArgsBrokerSetTimestampConfig.h"
#include "qpid/broker/manage/ArgsBrokerGetTimestampConfig.h"
#include "qpid/broker/manage/ArgsBrokerQueueRedirect.h"
...
#include "qpid/broker/ManagementDirectExchange.h"
#include "qpid/broker/ManagementTopicExchange.h"
...
using qmf2::Args;
using qpid::broker::getCurrentPublisher;
using qpid::types::Variant;
using std::string;
using std::make_pair;

namespace _qmf = qpid::broker::manage;
    ...
    sessionMaxUnacked(5000) 
{
    ...
    addOptions()
        ("data-dir", sys::optValue(dataDir,"DIR"), "Directory to contain persistent data generated by the broker")
        ("no-data-dir", sys::optValue(noDataDir), "Don't use a data directory.  No persistent configuration will be loaded or stored")
        ("paging-dir", sys::optValue(pagingDir,"DIR"), "Directory in which paging files will be created for paged queues")
        ("port,p", sys::optValue(port,"PORT"), "Tells the broker to listen on PORT")
        ("interface", sys::optValue(listenInterfaces, "<interface name>|<interface address>"), "Which network interfaces to use to listen for incoming connections")
        ("listen-disable", sys::optValue(listenDisabled, "<transport name>"), "Transports to disable listening")
        ("protocols", sys::optValue(protocols, "<protocol name+version>"), "Which protocol versions to allow")
        ("worker-threads", sys::optValue(workerThreads, "N"), "Sets the broker thread pool size")
        ("connection-backlog", sys::optValue(connectionBacklog, "N"), "Sets the connection backlog limit for the server socket")
        ("mgmt-enable,m", sys::optValue(enableMgmt,"yes|no"), "Enable Management")
        ("mgmt-publish", sys::optValue(mgmtPublish,"yes|no"), "Enable Publish of Management Data ('no' implies query-only)")
        ("mgmt-qmf2", sys::optValue(qmf2Support,"yes|no"), "Enable broadcast of management information over QMF v2")
        ("mgmt-qmf1", sys::optValue(qmf1Support,"yes|no"), "Enable broadcast of management information over QMF v1")
        ("mgmt-pub-interval", sys::optValue(mgmtPubInterval, "SECONDS"), "Management Publish Interval")
        ("queue-purge-interval", sys::optValue(queueCleanInterval, "SECONDS"),
         "Interval between attempts to purge any expired messages from queues")
        ("auth", sys::optValue(auth, "yes|no"), "Enable authentication, if disabled all incoming connections will be trusted")
        ("realm", sys::optValue(realm, "REALM"), "Use the given realm when performing authentication")
        ("sasl-service-name", sys::optValue(saslServiceName, "NAME"), "The service name to specify for SASL")
        ("default-queue-limit", sys::optValue(queueLimit, "BYTES"), "Default maximum size for queues (in bytes)")
        ("tcp-nodelay", sys::optValue(tcpNoDelay), "Set TCP_NODELAY on TCP connections")
        ("require-encryption", sys::optValue(requireEncrypted), "Only accept connections that are encrypted")
        ("known-hosts-url", sys::optValue(knownHosts, "URL or 'none'"), "URL to send as 'known-hosts' to clients ('none' implies empty list)")
        ("sasl-config", sys::optValue(saslConfigPath, "DIR"), "Allows SASL config path, if supported by platform, to be overridden.  For default location on Linux, see Cyrus SASL documentation.  There is no SASL config dir on Windows.")
        ("default-flow-stop-threshold", sys::optValue(queueFlowStopRatio, "PERCENT"), "Percent of queue's maximum capacity at which flow control is activated.")
        ("default-flow-resume-threshold", sys::optValue(queueFlowResumeRatio, "PERCENT"), "Percent of queue's maximum capacity at which flow control is de-activated.")
        ("default-event-threshold-ratio", sys::optValue(queueThresholdEventRatio, "%age of limit"), "The ratio of any specified queue limit at which an event will be raised")
        ("default-message-group", sys::optValue(defaultMsgGroup, "GROUP-IDENTIFER"), "Group identifier to assign to messages delivered to a message group queue that do not contain an identifier.")
        ("enable-timestamp", sys::optValue(timestampRcvMsgs, "yes|no"), "Add current time to each received message.")
        ("link-maintenance-interval", sys::optValue(linkMaintenanceInterval, "SECONDS"),
         "Interval to check federation link health and re-connect if need be")
        ("link-heartbeat-interval", sys::optValue(linkHeartbeatInterval, "SECONDS"),
         "Heartbeat interval for a federation link")
        ("dtx-default-timeout", sys::optValue(dtxDefaultTimeout, "SECONDS"), "Default timeout for DTX transaction before aborting it")
        ("dtx-max-timeout", sys::optValue(dtxMaxTimeout, "SECONDS"), "Maximum allowed timeout for DTX transaction. A value of zero disables maximum timeout limit checks and allows arbitrarily large timeout settings.")
        ("max-negotiate-time", sys::optValue(maxNegotiateTime, "MILLISECONDS"), "Maximum time a connection can take to send the initial protocol negotiation")
        ("federation-tag", sys::optValue(fedTag, "NAME"), "Override the federation tag")
        ("session-max-unacked", sys::optValue(sessionMaxUnacked, "DELIVERIES"), "Maximum number of un-acknowledged outoing messages per sesssion")
        ;
}
...
Broker::LogPrefix::LogPrefix() :
    std::string(sys::Msg() << "Broker (pid=" << sys::SystemInfo::getProcessId() << ") ") {
...
Broker::Broker(const BrokerOptions& conf) :
    ...
    sessionManager(
        qpid::base::SessionState::Configuration(
    ...
    getKnownBrokers(boost::bind(&Broker::getKnownBrokersImpl, this))
{
    ...
    if (conf.enableMgmt) {
        ...
        managementAgent->configure(dataDir.isEnabled() ? dataDir.getPath() : string(), conf.mgmtPublish,
                                   (uint16_t)(conf.mgmtPubInterval/sys::TIME_SEC), this, conf.workerThreads + 3);
    ...
    // Initialize plugins
    base::Plugin::initializeAll(*this);
    ...
    if (!conf.knownHosts.empty() && conf.knownHosts != knownHostsNone) {
        knownBrokers.push_back(sys::Url(conf.knownHosts));
    }
...
void Broker::setStore (const boost::shared_ptr<MessageStore>& _store)
{
    ...
    if (!NullMessageStore::isNullStore(store.get())) {
        ...
        throw qpid::sys::Exception(QPID_MSG("Failed to start broker: Multiple store plugins were loaded"));
    }
...
void Broker::run() {
    if (config.workerThreads > 0) {
        ...
        throw qpid::sys::Exception((boost::format("Invalid value for worker-threads: %1%") % config.workerThreads).str());
    }
}
...
struct InvalidBindingIdentifier : public qpid::sys::Exception
{
    InvalidBindingIdentifier(const std::string& name) : qpid::sys::Exception(name) {}
...
struct ObjectAlreadyExists : public qpid::sys::Exception
{
    ObjectAlreadyExists(const std::string& name) : qpid::sys::Exception(name) {}
    std::string getPrefix() const { return "object already exists"; }
};

struct UnknownObjectType : public qpid::sys::Exception
{
    UnknownObjectType(const std::string& type) : qpid::sys::Exception(type) {}
    std::string getPrefix() const { return "unknown object type"; }
};

struct ReservedObjectName : public qpid::sys::Exception
{
    ReservedObjectName(const std::string& type) : qpid::sys::Exception(type) {}
    std::string getPrefix() const { return std::string("names prefixed with '")
          + QPID_NAME_PREFIX + std::string("' are reserved"); }
};

struct UnsupportedTransport : public qpid::sys::Exception
{
    UnsupportedTransport(const std::string& type) : qpid::sys::Exception(type) {}
    std::string getPrefix() const { return "transport is not supported"; }
};

struct InvalidParameter : public qpid::sys::Exception
{
    InvalidParameter(const std::string& type) : qpid::sys::Exception(type) {}
                          ...
                          const Variant::Map& properties, bool /*strict*/, const Connection* context)
{
    ...
    if (objectFactory.createObject(*this, type, name, properties, userId, connectionId)) {
        ...
        settings.populate(extensions, unused);
         qpid::broker::amqp0_10::translate(unused, settings.storeSettings);
        ...
        framing::FieldTable arguments;
         qpid::broker::amqp0_10::translate(extensions, arguments);
        ...
        try {
            ...
            throw qpid::sys::Exception(QPID_MSG("Invalid exchange type: " << exchangeType));
        }
...
void Broker::accept() {
    ...
    if ( accepting==0 ) {
        throw qpid::sys::Exception(QPID_MSG("Failed to start broker: No transports are listening for incoming connections"));
    }
}
