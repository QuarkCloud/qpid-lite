#ifndef QPID_DRIVER_CONNECTION_IMPL_H
#define QPID_DRIVER_CONNECTION_IMPL_H 1
...
namespace qpid {
	namespace driver {
			...
			public boost::enable_shared_from_this<ConnectionImpl>
		{
			...
			void release();
			ConnectionImpl(framing::ProtocolVersion version, const amqp::ConnectionOptions& settings);

			boost::function<void()> failureCallback;

		public:
			static void init();
			static boost::shared_ptr<ConnectionImpl> create(framing::ProtocolVersion version, const amqp::ConnectionOptions& settings);
			~ConnectionImpl();
			...
			const amqp::ConnectionOptions& getNegotiatedSettings();

			std::vector<qpid::sys::Url> getInitialBrokers();
		...
		QPID_DRIVER_EXTERN void shutdown();

	}
}


#endif /**QPID_DRIVER_CONNECTION_IMPL_H*/
