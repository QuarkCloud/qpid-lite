#ifndef QPID_DRIVER_SUBSCRIPTION_MANAGER_H
#define QPID_DRIVER_SUBSCRIPTION_MANAGER_H 1
...
#include "qpid/driver/Subscription.h"
 ...
 *
 */
class QPID_DRIVER_CLASS_EXTERN SubscriptionManager : public sys::Runnable, public Handle<SubscriptionManagerImpl>
{
  public:
    /** Create a new SubscriptionManager associated with a session */
    QPID_DRIVER_EXTERN SubscriptionManager(const Session& session);
    QPID_DRIVER_EXTERN SubscriptionManager(const SubscriptionManager&);
    QPID_DRIVER_EXTERN ~SubscriptionManager();
    QPID_DRIVER_EXTERN SubscriptionManager& operator=(const SubscriptionManager&);
     ...
     */
    QPID_DRIVER_EXTERN Subscription subscribe(MessageListener& listener,
     ...
     */
    QPID_DRIVER_EXTERN Subscription subscribe(LocalQueue& localQueue,
                           const std::string& queue,
                           const SubscriptionSettings& settings,
                           const std::string& name=std::string());
     ...
     */
    QPID_DRIVER_EXTERN bool get(Message& result, const std::string& queue, sys::NanoDuration timeout=0);
     ...
     */
    QPID_DRIVER_EXTERN Message get(const std::string& queue, sys::NanoDuration timeout=sys::TIME_INFINITE);

    /** Get a subscription by name.
     *@throw Exception if not found.
     */
    QPID_DRIVER_EXTERN Subscription getSubscription(const std::string& name) const;

    /** Cancel a subscription. See also: Subscription.cancel() */
    QPID_DRIVER_EXTERN void cancel(const std::string& name);
     ...
     * @see run
     */
    QPID_DRIVER_EXTERN void run();

    /** Start a new thread to deliver messages.
     * Only one thread may be running in a SubscriptionManager at a time.
     * @see start
     */
    QPID_DRIVER_EXTERN void start();
     ...
     */
    QPID_DRIVER_EXTERN void setAutoStop(bool set=true);

    /** Stop delivery. Causes run() to return, or the thread started with start() to exit. */
    QPID_DRIVER_EXTERN void stop();

    static const uint32_t UNLIMITED=0xFFFFFFFF;

    /** Set the flow control for a subscription. */
    QPID_DRIVER_EXTERN void setFlowControl(const std::string& name, const FlowControl& flow);
     ...
     */
    QPID_DRIVER_EXTERN void setFlowControl(const std::string& name, uint32_t messages,  uint32_t bytes, bool window=true);

    /** Set the default settings for subscribe() calls that don't
     * include a SubscriptionSettings parameter.
     */
    QPID_DRIVER_EXTERN void setDefaultSettings(const SubscriptionSettings& s);

    /** Get the default settings for subscribe() calls that don't
     * include a SubscriptionSettings parameter.
     */
    QPID_DRIVER_EXTERN const SubscriptionSettings& getDefaultSettings() const;

    /** Get the default settings for subscribe() calls that don't
     * include a SubscriptionSettings parameter.
     */
    QPID_DRIVER_EXTERN SubscriptionSettings& getDefaultSettings();
     ...
     *@param window: if true use window-based flow control.
     */
    QPID_DRIVER_EXTERN void setFlowControl(uint32_t messages,  uint32_t bytes, bool window=true);

    /**
     *Set the default accept-mode for subscribe() calls that don't
     *include a SubscriptionSettings parameter.
     */
    QPID_DRIVER_EXTERN void setAcceptMode(AcceptMode mode);

    /**
     * Set the default acquire-mode subscribe()s that don't specify SubscriptionSettings.
     */
    QPID_DRIVER_EXTERN void setAcquireMode(AcquireMode mode);

    QPID_DRIVER_EXTERN void registerFailoverHandler ( boost::function<void ()> fh );
...
#endif  /*!QPID_DRIVER_SUBSCRIPTION_MANAGER_H*/
