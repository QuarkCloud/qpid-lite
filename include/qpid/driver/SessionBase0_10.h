#ifndef QPID_DRIVER_SESSION_BASE_H
#define QPID_DRIVER_SESSION_BASE_H 1
...
#include "qpid/driver/TypedResult.h"
...
class QPID_DRIVER_CLASS_EXTERN SessionBase0_10 {
  public:
     ...
     */
    QPID_DRIVER_EXTERN void sync();

    /** Set the timeout for this session. */
    QPID_DRIVER_EXTERN uint32_t timeout(uint32_t seconds);

    /** Suspend the session - detach it from its connection */
    QPID_DRIVER_EXTERN void suspend();

    /** Resume a suspended session with a new connection */
    QPID_DRIVER_EXTERN void resume(Connection);

    /** Get the channel associated with this session */
    QPID_DRIVER_EXTERN uint16_t getChannel() const;

    QPID_DRIVER_EXTERN void flush();
    QPID_DRIVER_EXTERN void markCompleted(const framing::SequenceSet& ids, bool notifyPeer);
    QPID_DRIVER_EXTERN void markCompleted(const framing::SequenceNumber& id, bool cumulative, bool notifyPeer);
    QPID_DRIVER_EXTERN void sendCompletion();

    QPID_DRIVER_EXTERN bool isValid() const;
...
#endif  /*!QPID_DRIVER_SESSION_BASE_H*/
