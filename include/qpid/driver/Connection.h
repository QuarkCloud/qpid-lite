#ifndef QPID_DRIVER_CONNECTION_H
#define QPID_DRIVER_CONNECTION_H 1
...
class QPID_DRIVER_CLASS_EXTERN Connection
{
  ...
  public:
     ...
     */
    QPID_DRIVER_EXTERN void open(const std::string& host, int port = 5672,
     ...
     * within a single broker).
     */
    QPID_DRIVER_EXTERN void open(const sys::Url& url,
              const std::string& uid = "",
              const std::string& pwd = "",
              const std::string& virtualhost = "/", uint16_t maxFrameSize=65535);
     ...
     */
    QPID_DRIVER_EXTERN void open(const sys::Url& url, const amqp::ConnectionOptions& settings);
     ...
     */
    QPID_DRIVER_EXTERN void open(const amqp::ConnectionOptions& settings);
     ...
     */
    QPID_DRIVER_EXTERN Session newSession(const std::string& name=std::string(), uint32_t timeoutSeconds = 0);

    /**
     * Resume a suspended session. A session may be resumed
     * on a different connection to the one that created it.
     */
    QPID_DRIVER_EXTERN void resume(Session& session);

    QPID_DRIVER_EXTERN bool isOpen() const;

    /** In a cluster, returns the initial set of known broker URLs
     * at the time of connection.
     */
    QPID_DRIVER_EXTERN std::vector<sys::Url> getInitialBrokers();

    QPID_DRIVER_EXTERN void registerFailureCallback ( boost::function<void ()> fn );

    /**
     * Return the set of driver negotiated settings
     */
...
#endif  /*!QPID_DRIVER_CONNECTION_H*/
