#ifndef QPID_DRIVER_LOCAL_QUEUE_H
#define QPID_DRIVER_LOCAL_QUEUE_H 1
...
class QPID_DRIVER_CLASS_EXTERN LocalQueue : public Handle<LocalQueueImpl> {
  public:
     ...
     */
    QPID_DRIVER_EXTERN LocalQueue();
    QPID_DRIVER_EXTERN LocalQueue(const LocalQueue&);
    QPID_DRIVER_EXTERN ~LocalQueue();
    QPID_DRIVER_EXTERN LocalQueue& operator=(const LocalQueue&);
     ...
     */
    QPID_DRIVER_EXTERN bool get(Message& result, sys::NanoDuration timeout=0);
     ...
     */
    QPID_DRIVER_EXTERN Message get(sys::NanoDuration timeout=sys::TIME_INFINITE);

    /** Synonym for get() */
    QPID_DRIVER_EXTERN Message pop(sys::NanoDuration timeout=sys::TIME_INFINITE);

    /** Return true if local queue is empty. */
    QPID_DRIVER_EXTERN bool empty() const;

    /** Number of messages on the local queue */
    QPID_DRIVER_EXTERN size_t size() const;
...
#endif  /*!QPID_DRIVER_LOCAL_QUEUE_H*/
