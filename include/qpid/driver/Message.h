#ifndef QPID_DRIVER_MESSAGE_H
#define QPID_DRIVER_MESSAGE_H 1
 ...
 *
 */
class QPID_DRIVER_CLASS_EXTERN Message
{
public:
    /** Create a Message.
     ...
     */
    QPID_DRIVER_EXTERN Message(
        const std::string& data=std::string(),
        const std::string& routingKey=std::string());
    Message(MessageImpl*);    ///< @internal
    QPID_DRIVER_EXTERN Message(const Message&);
    QPID_DRIVER_EXTERN ~Message();
    QPID_DRIVER_EXTERN Message& operator=(const Message&);
    QPID_DRIVER_EXTERN void swap(Message&);

    QPID_DRIVER_EXTERN void setData(const std::string&);
    QPID_DRIVER_EXTERN const std::string& getData() const;
    QPID_DRIVER_EXTERN std::string& getData();

    QPID_DRIVER_EXTERN void appendData(const std::string&);

    QPID_DRIVER_EXTERN bool hasMessageProperties() const;
    QPID_DRIVER_EXTERN framing::MessageProperties& getMessageProperties();
    QPID_DRIVER_EXTERN const framing::MessageProperties& getMessageProperties() const;

    QPID_DRIVER_EXTERN bool hasDeliveryProperties() const;
    QPID_DRIVER_EXTERN framing::DeliveryProperties& getDeliveryProperties();
    QPID_DRIVER_EXTERN const framing::DeliveryProperties& getDeliveryProperties() const;
     ...
     */
    QPID_DRIVER_EXTERN std::string getDestination() const;

    /** Check the redelivered flag. */
    QPID_DRIVER_EXTERN bool isRedelivered() const;
    /** Set the redelivered flag. */
    QPID_DRIVER_EXTERN void setRedelivered(bool redelivered);

    /** Get a modifyable reference to the message headers. */
    QPID_DRIVER_EXTERN framing::FieldTable& getHeaders();

    /** Get a non-modifyable reference to the message headers. */
    QPID_DRIVER_EXTERN const framing::FieldTable& getHeaders() const;

    // FIXME aconway 2009-04-17: does this need to be in public API?
    ///@internal
    QPID_DRIVER_EXTERN const framing::SequenceNumber& getId() const;
