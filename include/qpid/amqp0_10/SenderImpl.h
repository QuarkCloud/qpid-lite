#ifndef QPID_AMQP0_10_SENDER_IMPL_H
	...
	namespace amqp0_10 {
		...
		public:
			...
			SenderImpl(SessionImpl& parent, const std::string& name,
				const qpid::amqp::Address& address, bool autoReconnect);
			void send(const qpid::amqp::Message&, bool sync);
			void close();
		...
		private:
			...
			void sendImpl(const qpid::amqp::Message&);
			void sendUnreliable(const qpid::amqp::Message&);
			void closeImpl();
			...
			struct Send : Command
			{
				...
				Send(SenderImpl& i, const qpid::amqp::Message& m) : Command(i), message(m), repeat(true) {}
				void operator()()
				{
			...
			struct UnreliableSend : Command
			{
				...
				UnreliableSend(SenderImpl& i, const qpid::amqp::Message& m) : Command(i), message(m) {}
				void operator()()
				{
...
#endif  /*!QPID_AMQP0_10_SENDER_IMPL_H*/
