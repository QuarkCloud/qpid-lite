...
struct Statement {
    ...
    QPID_LOG_EXTERN void log(const std::string& message);
	QPID_LOG_EXTERN static void categorize(Statement& s);

    struct Initializer {
		QPID_LOG_EXTERN Initializer(Statement& s);
		QPID_LOG_EXTERN ~Initializer();
        Statement& statement;
    };
};
