...
struct SelectorElement {
    QPID_LOG_EXTERN SelectorElement(const std::string cliEntry);
    std::string levelStr;
...
class Selector {
  public:
	...
	QPID_LOG_EXTERN Selector();

    /** Set selector from Options */
	QPID_LOG_EXTERN Selector(const Options&);

    /** Equavlient to: Selector s; s.enable(l, s) */
	QPID_LOG_EXTERN Selector(Level l, const std::string& s=std::string());

    /** Selector from string */
	QPID_LOG_EXTERN Selector(const std::string& selector);

    /** push option settings into runtime lookup structs */
	QPID_LOG_EXTERN void enable(const std::string& enableStr);
	QPID_LOG_EXTERN void disable(const std::string& disableStr);

    /**
     * Enable/disable messages with level in levels where the file
     * name contains substring. Empty string matches all.
     */
	QPID_LOG_EXTERN void enable(Level level, const std::string& substring=std::string());
	QPID_LOG_EXTERN void disable(Level level, const std::string& substring=std::string());

    /** Tests to determine if function names are in enable/disable tables */
	QPID_LOG_EXTERN bool isEnabled(Level level, const char* function);
	QPID_LOG_EXTERN bool isDisabled(Level level, const char* function);

    /** Test to determine if log Statement is enabled */
	QPID_LOG_EXTERN bool isEnabled(Level level, const char* function, Category category);
