...
namespace log {
	...
	using qpid::sys::optValue;

/** Logging options for config parser. */
struct Options : public qpid::sys::Options {
    /** Pass argv[0] for use in syslog output */
    QPID_LOG_EXTERN Options(const std::string& argv0_=std::string(),
            const std::string& name_="Logging options");
	QPID_LOG_EXTERN Options(const Options &);

	QPID_LOG_EXTERN Options& operator=(const Options&);
...
QPID_LOG_EXTERN std::string getLevels();

/** Get a string list of the allowed categories */
QPID_LOG_EXTERN std::string getCategories();
