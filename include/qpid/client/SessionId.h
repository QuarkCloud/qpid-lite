...
namespace qpid {
	namespace client {
		...
		class SessionId : private boost::totally_ordered1<SessionId> {
			...
			std::string name;
		public:
			QPID_CLIENT_EXTERN SessionId(const std::string& userId = std::string(), const std::string& name = std::string());
			std::string getUserId() const { return userId; }
			std::string getName() const { return name; }
			QPID_CLIENT_EXTERN bool operator<(const SessionId&) const;
			QPID_CLIENT_EXTERN bool operator==(const SessionId& id) const;
			// Convert to a string
			QPID_CLIENT_EXTERN std::string str() const;
		};

		QPID_CLIENT_EXTERN std::ostream& operator<<(std::ostream&, const SessionId&);
