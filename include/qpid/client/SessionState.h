#ifndef QPID_CLIENT_SESSION_STATE_H
#define QPID_CLIENT_SESSION_STATE_H 1
...
namespace qpid {
	namespace client {
		...
		struct SessionPoint : boost::totally_ordered1<SessionPoint> {
			QPID_CLIENT_EXTERN SessionPoint(SequenceNumber command = 0, uint64_t offset = 0);

			SequenceNumber command;
			uint64_t offset;

			/** Advance past frame f */
			QPID_CLIENT_EXTERN void advance(const framing::AMQFrame& f);

			QPID_CLIENT_EXTERN bool operator<(const SessionPoint&) const;
			QPID_CLIENT_EXTERN bool operator==(const SessionPoint&) const;
		};

		QPID_CLIENT_EXTERN std::ostream& operator<<(std::ostream&, const SessionPoint&);
		...
		public:
			...
			struct Configuration {
				QPID_CLIENT_EXTERN Configuration(size_t flush = 1024 * 1024, size_t hard = 0);
			...
			QPID_CLIENT_EXTERN SessionState(const SessionId& = SessionId(), const Configuration& = Configuration());

			QPID_CLIENT_EXTERN virtual ~SessionState();

			QPID_CLIENT_EXTERN bool hasState() const;

			QPID_CLIENT_EXTERN const SessionId& getId() const { return id; }

			QPID_CLIENT_EXTERN virtual uint32_t getTimeout() const;
			QPID_CLIENT_EXTERN virtual void setTimeout(uint32_t seconds);
			...
			QPID_CLIENT_EXTERN virtual void senderRecord(const framing::AMQFrame& f);

			/** @return true if we should send flush for confirmed and completed commands. */
			QPID_CLIENT_EXTERN virtual bool senderNeedFlush() const;

			/** Called when flush for confirmed and completed commands is sent to peer. */
			QPID_CLIENT_EXTERN virtual void senderRecordFlush();

			/** True if we should reply to the next incoming completed command */
			QPID_CLIENT_EXTERN virtual bool senderNeedKnownCompleted() const;

			/** Called when knownCompleted is sent to peer. */
			QPID_CLIENT_EXTERN virtual void senderRecordKnownCompleted();

			/** Called when the peer confirms up to comfirmed. */
			QPID_CLIENT_EXTERN virtual void senderConfirmed(const SessionPoint& confirmed);

			/** Called when the peer indicates commands completed */
			QPID_CLIENT_EXTERN virtual void senderCompleted(const SequenceSet& commands);

			/** Point from which the next new (not replayed) data will be sent. */
			QPID_CLIENT_EXTERN virtual SessionPoint senderGetCommandPoint();

			/** Set of outstanding incomplete commands */
			QPID_CLIENT_EXTERN virtual SequenceSet senderGetIncomplete() const;

			/** Point from which we can replay. */
			QPID_CLIENT_EXTERN virtual SessionPoint senderGetReplayPoint() const;

			/** Peer expecting commands from this point.
			 *@return Range of frames to be replayed.
			 */
			QPID_CLIENT_EXTERN virtual ReplayRange senderExpected(const SessionPoint& expected);

			// ==== Functions for receiver state

			/** Set the command point. */
			QPID_CLIENT_EXTERN virtual void receiverSetCommandPoint(const SessionPoint& point);

			/** Returns true if frame should be be processed, false if it is a duplicate. */
			QPID_CLIENT_EXTERN virtual bool receiverRecord(const framing::AMQFrame& f);

			/** Command completed locally */
			QPID_CLIENT_EXTERN virtual void receiverCompleted(SequenceNumber command, bool cumulative = false);

			/** Peer has indicated commands are known completed */
			QPID_CLIENT_EXTERN virtual void receiverKnownCompleted(const SequenceSet& commands);

			/** True if the next completed control should set the timely-reply argument
			 * to request a knonw-completed response.
			 */
			QPID_CLIENT_EXTERN virtual bool receiverNeedKnownCompleted() const;

			/** Get the incoming command point */
			QPID_CLIENT_EXTERN virtual const SessionPoint& receiverGetExpected() const;

			/** Get the received high-water-mark, may be > getExpected() during replay */
			QPID_CLIENT_EXTERN virtual const SessionPoint& receiverGetReceived() const;

			/** Completed received commands that the peer may not know about. */
			QPID_CLIENT_EXTERN virtual const SequenceSet& receiverGetUnknownComplete() const;

			/** Incomplete received commands. */
			QPID_CLIENT_EXTERN virtual const SequenceSet& receiverGetIncomplete() const;

			/** ID of the command currently being handled. */
			QPID_CLIENT_EXTERN virtual SequenceNumber receiverGetCurrent() const;

			/** Set the state variables, used to create a session that will resume
			 *  from some previously established point.
			 */
			QPID_CLIENT_EXTERN virtual void setState(
			 ...
			 */
			QPID_CLIENT_EXTERN void disableReceiverTracking();
			QPID_CLIENT_EXTERN void enableReceiverTracking();
...
#endif  /*!QPID_CLIENT_SESSION_STATE_H*/
