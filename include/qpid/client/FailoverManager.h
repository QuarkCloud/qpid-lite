...
struct CannotConnectException : qpid::sys::Exception
{
    CannotConnectException(const std::string& m) : qpid::sys::Exception(m) {}
};
...
class QPID_CLIENT_CLASS_EXTERN FailoverManager
{
  public:
    ...
    struct ReconnectionStrategy
    {
         ...
         * last connection.
         */
        virtual void editUrlList(std::vector<sys::Url>&  urls) = 0;
     ...
     * attempted
     */
    QPID_CLIENT_EXTERN FailoverManager(const amqp::ConnectionOptions& settings, ReconnectionStrategy* strategy = 0);
     ...
     */
    QPID_CLIENT_EXTERN Connection& connect(std::vector<sys::Url> brokers = std::vector<sys::Url>());
    /**
  ...
  private:
    ...
    void attempt(Connection&, amqp::ConnectionOptions settings, std::vector<sys::Url> urls);
    void attempt(Connection&, amqp::ConnectionOptions settings);
};
