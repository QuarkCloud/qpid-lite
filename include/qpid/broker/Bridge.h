 ...
 *
 */
#ifndef QPID_BROKER_BRIDGE_H
#define QPID_BROKER_BRIDGE_H 1
	...
	namespace broker {
		...
		public:
			...
			Bridge(const std::string& name, Link* link, framing::ChannelId id, CancellationListener l,
				const qpid::broker::manage::ArgsLinkBridge& args,
			...
			qmf2::Manageable::status_t ManagementMethod(uint32_t methodId,
				::qmf2::Args& args,
				std::string& text);
			...
			const qpid::broker::manage::ArgsLinkBridge& getArgs() { return args; }
		...
		private:
			struct PushHandler : framing::FrameHandler {
				PushHandler(amqp0_10::Connection* c) { conn = c; }
				void handle(framing::AMQFrame& frame);
				amqp0_10::Connection* conn;
			};
			...
			qpid::broker::manage::ArgsLinkBridge args;
			qpid::broker::manage::Bridge::shared_ptr mgmtObject;
			...
			void create(amqp0_10::Connection& c);
			void cancel(amqp0_10::Connection& c);
			void closed();
