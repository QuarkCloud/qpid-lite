#ifndef QPID_BROKER_ACL_ACLDATA_H
#define QPID_BROKER_ACL_ACLDATA_H 1
...
#include "qpid/broker/acl/AclTopicMatch.h"
	...
	namespace broker {
		namespace acl {
			...
			public:
				...
				const base::AclHost& getAclHost() const { return aclHost; }
				const AclResult& getAclResult() const { return aclResult; }

			private:
				AclResult   aclResult;
				base::AclHost     aclHost;
			};
			...
			public:
				...
				typedef std::map<qpid::broker::acl::Property, std::string> propertyMap;
				typedef propertyMap::const_iterator                propertyMapItr;

				typedef std::map<qpid::broker::acl::SpecProperty, std::string> specPropertyMap;
				...
				struct Rule {
					...
					qpid::broker::acl::AclResult  ruleMode;     // combined allow/deny log/nolog
					...
					Rule(int ruleNum, qpid::broker::acl::AclResult res, specPropertyMap& p) :
						rawRuleNum(ruleNum),
						ruleMode(res),
						props(p),
				...
				aclAction*           actionList[qpid::broker::acl::ACTIONSIZE];
				qpid::broker::acl::AclResult decisionMode;  // allow/deny[-log] if no matching rule found
				bool                 transferAcl;
				std::string          aclSource;
				qpid::broker::acl::AclResult connectionDecisionMode;
			...
			private:
				...
				bool compareInt(const qpid::broker::acl::SpecProperty theProperty,
...
#endif // QPID_BROKER_ACL_ACLDATA_H
