 ...
 */
class SessionState : public qpid::base::SessionState,
                     public SessionContext,
                     public qmf2::Manageable,
                     public framing::FrameHandler::InOutHandler
{
  public:
    SessionState(Broker&, SessionHandler&, const base::SessionId&,
    ...
    /** @pre isAttached() */
    amqp0_10::Connection& getConnection();
    ...
    DeliveryId deliver(const qpid::broker::amqp0_10::MessageTransfer& message,
    ...
    qmf2::Manageable::status_t
    ManagementMethod (uint32_t methodId, qmf2::Args& args, std::string&);

    void readyToSend();

    const base::SessionId& getSessionId() const { return getId(); }
     ...
     */
    bool addPendingExecutionSync(framing::SequenceNumber id);
    ...
    void completeCommand(framing::SequenceNumber id, bool requiresAccept, bool requiresSync,
  ...
  private:
    ...
    qpid::broker::manage::Session::shared_ptr mgmtObject;
    qpid::framing::SequenceSet accepted;

    // sequence numbers for pending received Execution.Sync commands
    std::queue<framing::SequenceNumber> pendingExecutionSyncs;
  ...
  public:
    ...
    class CurrentCommand {
      public:
        CurrentCommand(
			framing::SequenceNumber id_=0, bool syncRequired_=false, bool completeSync_=true ) :
      ...
      private:
		framing::SequenceNumber id;   ///< Command identifier.
     ...
     * SessionState.
     */
    class AsyncCommandCompleter : public sys::RefCounted {
    private:
        ...
        struct CommandInfo {
			framing::SequenceNumber cmd; // message.transfer command id
            bool requiresAccept;
            bool requiresSync;

            CommandInfo(
				framing::SequenceNumber c, bool a, bool s)
        ...
        std::map<framing::SequenceNumber, boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> > pendingMsgs;
    ...
    public:
        ...
        void addPendingMessage(boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> m);
        void deletePendingMessage(framing::SequenceNumber id);
        void flushPendingMessages();
        /** schedule the processing of command completion. */
        void scheduleCommandCompletion(framing::SequenceNumber cmd,
                                       bool requiresAccept,
                                       bool requiresSync);
    ...
    class IncompleteIngressMsgXfer : public SessionState::AsyncCommandContext
    {
     public:
        IncompleteIngressMsgXfer( SessionState *ss,
                                  boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> m)
     ...
     private:
        ...
        boost::intrusive_ptr<qpid::broker::amqp0_10::MessageTransfer> msg;
        bool requiresAccept;
        bool requiresSync;
