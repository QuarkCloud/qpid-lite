...
#ifndef QPID_BROKER_MANAGE_EVENT_QUEUE_REDIRECT_CANCELLED_H
#define QPID_BROKER_MANAGE_EVENT_QUEUE_REDIRECT_CANCELLED_H 1
...
namespace qpid {
	namespace broker {
		namespace manage {
			...
			QPID_BROKER_CLASS_EXTERN class EventQueueRedirectCancelled : public ::qmf2::ManagementEvent
			{
			private:
				...
				const std::string& qTarget;


			public:
				writeSchemaCall_t getWriteSchemaCall(void) { return writeSchema; }

				QPID_BROKER_EXTERN EventQueueRedirectCancelled(const std::string& _qName,
					const std::string& _qTarget);
				QPID_BROKER_EXTERN ~EventQueueRedirectCancelled() {};
...
#endif  /*!QPID_BROKER_MANAGE_EVENT_QUEUE_REDIRECT_CANCELLED_H*/
