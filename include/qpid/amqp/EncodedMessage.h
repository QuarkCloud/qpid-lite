#ifndef QPID_AMQP_ENCODED_MESSAGE_H
#define QPID_AMQP_ENCODED_MESSAGE_H 1
...
namespace qpid {
	namespace amqp {
		...
		{
		public:
			QPID_AMQP_EXTERN EncodedMessage();
			QPID_AMQP_EXTERN EncodedMessage(size_t);
			QPID_AMQP_EXTERN EncodedMessage(const EncodedMessage&);
			QPID_AMQP_EXTERN ~EncodedMessage();


			QPID_AMQP_EXTERN size_t getSize() const;
			QPID_AMQP_EXTERN char* getData();
			QPID_AMQP_EXTERN const char* getData() const;
			QPID_AMQP_EXTERN void trim(size_t);
			QPID_AMQP_EXTERN void resize(size_t);

			QPID_AMQP_EXTERN void setNestAnnotationsOption(bool);
			void getReplyTo(qpid::amqp::Address&) const;
			...
			QPID_AMQP_EXTERN void init(qpid::amqp::MessageImpl&);
			QPID_AMQP_EXTERN qpid::amqp::CharSequence getBareMessage() const;
			qpid::amqp::CharSequence getBody() const;
			QPID_AMQP_EXTERN bool hasHeaderChanged(const qpid::amqp::MessageImpl&) const;
		private:
			size_t size;
			char* data;
			...
			{
			public:
				InitialScan(EncodedMessage& e, qpid::amqp::MessageImpl& m);
				//header:
...
#endif  /*!QPID_AMQP_ENCODED_MESSAGE_H*/
