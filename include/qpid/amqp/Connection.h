#ifndef QPID_AMQP_CONNECTION_H
#define QPID_AMQP_CONNECTION_H 1
...
namespace qpid {
	namespace amqp {
		...
		class QPID_AMQP_CLASS_EXTERN Connection : public qpid::amqp::Handle<ConnectionImpl>
		{
		public:
			QPID_AMQP_EXTERN Connection(ConnectionImpl* impl);
			QPID_AMQP_EXTERN Connection(const Connection&);
			QPID_AMQP_EXTERN Connection();
			/**
			 ...
			 */
			QPID_AMQP_EXTERN Connection(const std::string& url, const qpid::types::Variant::Map& options = qpid::types::Variant::Map());
			/**
			 ...
			 */
			QPID_AMQP_EXTERN Connection(const std::string& url, const std::string& options);
			QPID_AMQP_EXTERN ~Connection();
			QPID_AMQP_EXTERN Connection& operator=(const Connection&);
			QPID_AMQP_EXTERN void setOption(const std::string& name, const qpid::types::Variant& value);
			QPID_AMQP_EXTERN void open();
			QPID_AMQP_EXTERN bool isOpen();
			QPID_AMQP_EXTERN bool isOpen() const;
			 ...
			 * that.
			 */
			QPID_AMQP_EXTERN void reconnect(const std::string& url);
			/**
			...
			QPID_AMQP_EXTERN Session createTransactionalSession(const std::string& name = std::string());
			QPID_AMQP_EXTERN Session createSession(const std::string& name = std::string());

			QPID_AMQP_EXTERN Session getSession(const std::string& name) const;
			QPID_AMQP_EXTERN std::string getAuthenticatedUsername();

		private:
			friend class qpid::amqp::PrivateImplRef<Connection>;
		};
	}
}// namespace qpid::amqp

#endif  /*!QPID_AMQP_CONNECTION_H*/
