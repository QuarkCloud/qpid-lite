#ifndef QPID_MESSAGING_AMQP_SASL_SERVER_H
#define QPID_MESSAGING_AMQP_SASL_SERVER_H 1
...
namespace qpid {
	namespace messaging {
		namespace amqp {
			...
			{
			public:
				QPID_MESSAGING_EXTERN SaslServer(const std::string& id);
				QPID_MESSAGING_EXTERN virtual ~SaslServer();
				...
				QPID_MESSAGING_EXTERN void mechanisms(const std::string& mechanisms);
				QPID_MESSAGING_EXTERN void challenge(const std::string*);
				QPID_MESSAGING_EXTERN void completed(bool succeeded);
...
#endif  /*!QPID_MESSAGING_AMQP_SASL_SERVER_H*/
