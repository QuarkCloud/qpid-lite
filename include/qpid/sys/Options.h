#ifndef QPID_SYS_OPTIONS_H
#define QPID_SYS_OPTIONS_H 1
...
namespace qpid {
	namespace sys {
		...
		///@internal
		QPID_SYS_EXTERN std::string prettyArg(const std::string&, const std::string&);
		...
		QPID_SYS_EXTERN po::value_semantic* optValue(bool& value);
		QPID_SYS_EXTERN po::value_semantic* pure_switch(bool& value);
		...
		public:
			QPID_SYS_EXTERN options_description_easy_init(po::options_description* o);

			QPID_SYS_EXTERN options_description_easy_init&
		...
		struct Options {
			friend QPID_SYS_EXTERN std::ostream& operator<<(std::ostream& os, const Options&);

			struct Exception : public qpid::sys::Exception {
				Exception(const std::string& msg) : qpid::sys::Exception(msg) {}
			};

			QPID_SYS_EXTERN explicit Options(const std::string& name = std::string());
			...
			QPID_SYS_EXTERN options_description_easy_init addOptions();
			QPID_SYS_EXTERN void add(Options&);
			QPID_SYS_EXTERN const std::vector< boost::shared_ptr<po::option_description> >& options() const;
			QPID_SYS_EXTERN bool find_nothrow(const std::string&, bool);
			QPID_SYS_EXTERN void print(std::ostream& os);

		private:
			boost::shared_ptr<po::options_description> poOptions;
		};

		QPID_SYS_EXTERN std::ostream& operator<<(std::ostream& os, const Options&);

		/**
		 * Standard options for configuration
		 */
		struct CommonOptions : public Options {
			QPID_SYS_EXTERN CommonOptions(const std::string& name = std::string(),
				const std::string& configfile = std::string(),
