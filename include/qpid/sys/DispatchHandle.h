#ifndef QPID_SYS_DISPATCHER_HANDLE_H
#define QPID_SYS_DISPATCHER_HANDLE_H 1
...
public:
     ...
     */
    QPID_SYS_EXTERN DispatchHandle(const IOHandle& h, Callback rCb, Callback wCb, Callback dCb);
	QPID_SYS_EXTERN ~DispatchHandle();

    /** Add this DispatchHandle to the poller to be watched. */
	QPID_SYS_EXTERN void startWatch(Poller::shared_ptr poller);

    /** Resume watching for all non-0 callbacks. */
	QPID_SYS_EXTERN void rewatch();
    /** Resume watching for read only. */
	QPID_SYS_EXTERN void rewatchRead();

    /** Resume watching for write only. */
	QPID_SYS_EXTERN void rewatchWrite();

    /** Stop watching temporarily. The DispatchHandle remains
        associated with the poller and can be re-activated using
        rewatch. */
	QPID_SYS_EXTERN void unwatch();
    /** Stop watching for read */
	QPID_SYS_EXTERN void unwatchRead();
    /** Stop watching for write */
	QPID_SYS_EXTERN void unwatchWrite();

    /** Stop watching permanently. Disassociates from the poller. */
	QPID_SYS_EXTERN void stopWatch();
     ...
     */
	QPID_SYS_EXTERN void call(Callback iCb);

protected:
	QPID_SYS_EXTERN void doDelete();

private:
	QPID_SYS_EXTERN void processEvent(Poller::EventType dir);
};
...
#endif // QPID_SYS_DISPATCHER_HANDLE_H
