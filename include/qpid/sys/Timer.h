 ...
 *
 */
#ifndef QPID_SYS_TIMER_H
#define QPID_SYS_TIMER_H 1
...
class TimerTask : public RefCounted {
  ...
  public:
     ...
     */
	QPID_SYS_EXTERN TimerTask(AbsTime fireTime, const std::string& name);

	QPID_SYS_EXTERN virtual ~TimerTask();
     ...
     */
	QPID_SYS_EXTERN void setupNextFire();
...
class Timer : private Runnable {
  ...
  public:
	QPID_SYS_EXTERN Timer();
	QPID_SYS_EXTERN virtual ~Timer();
     ...
     */
	QPID_SYS_EXTERN virtual void add(boost::intrusive_ptr<TimerTask> task);

    /** Start the Timer
     *
     * This will start a new thread that runs the Timer and the fire callbacks.
     */
	QPID_SYS_EXTERN virtual void start();

    /** Stop the Timer
     *
     * This will stop the Timer and its thread.
     */
	QPID_SYS_EXTERN virtual void stop();
