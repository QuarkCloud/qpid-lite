...
class Shlib {
  public:
    ...
    QPID_SYS_EXTERN void unload();

    /** Look up symbol. */
	QPID_SYS_EXTERN void* getSymbol(const char* symbol);
  ...
  private:
    void* handle;
	QPID_SYS_EXTERN void load(const char* libname);
};
...
class AutoShlib : public Shlib {
  public:
    ...
    /** Calls unload() */
	QPID_SYS_EXTERN ~AutoShlib() throw();
};
