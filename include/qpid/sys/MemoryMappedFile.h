#ifndef QPID_SYS_MEMORY_MAPPED_FILE_H
#define QPID_SYS_MEMORY_MAPPED_FILE_H 1
...
class MemoryMappedFile {
  public:
    QPID_SYS_EXTERN MemoryMappedFile();
	QPID_SYS_EXTERN ~MemoryMappedFile();
    /**
     * Opens a file that can be mapped by region into memory
     */
	QPID_SYS_EXTERN void open(const std::string& name, const std::string& directory);
    /**
     * Closes and removes the file that can be mapped by region into memory
     */
	QPID_SYS_EXTERN void close();
    /**
     * Returns the page size
     */
	QPID_SYS_EXTERN size_t getPageSize();
    /**
     * Load a portion of the file into memory
     */
	QPID_SYS_EXTERN char* map(size_t offset, size_t size);
    /**
     * Evict a portion of the file from memory
     */
	QPID_SYS_EXTERN void unmap(char* region, size_t size);
    /**
     * Flush any changes to a previously mapped region of the file
     * back to disk
     */
	QPID_SYS_EXTERN void flush(char* region, size_t size);
    /**
     * Expand the capacity of the file
     */
	QPID_SYS_EXTERN void expand(size_t offset);
    /**
     * Returns true if memory mapping is supported, false otherwise
     */
	QPID_SYS_EXTERN static bool isSupported();
  private:
...
#endif  /*!QPID_SYS_MEMORY_MAPPED_FILE_H*/
