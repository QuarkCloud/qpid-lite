 ...
 *
 */
#ifndef QPID_SYS_AGGREGATE_OUTPUT_H
#define QPID_SYS_AGGREGATE_OUTPUT_H 1
...
class QPID_SYS_CLASS_EXTERN AggregateOutput : public OutputTask
{
  ...
  public:
    QPID_SYS_EXTERN AggregateOutput();

    // These may be called concurrently with any function.
	QPID_SYS_EXTERN void addOutputTask(OutputTask* t);

    // These functions must not be called concurrently with each other.
	QPID_SYS_EXTERN bool doOutput();
	QPID_SYS_EXTERN void removeOutputTask(OutputTask* t);
	QPID_SYS_EXTERN void removeAll();
...
#endif /** QPID_SYS_AGGREGATE_OUTPUT_H */
