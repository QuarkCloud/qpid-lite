#ifndef QPID_SYS_STATEMENT_H
#define QPID_SYS_STATEMENT_H 1
...
struct Statement {
    ...
    QPID_SYS_EXTERN void log(const std::string& message);
	QPID_SYS_EXTERN static void categorize(Statement& s);

    struct Initializer {
		QPID_SYS_EXTERN Initializer(Statement& s);
		QPID_SYS_EXTERN ~Initializer();
        Statement& statement;
    };
};
...
{ 0, __FILE__, __LINE__,  BOOST_CURRENT_FUNCTION, (::qpid::sys::LEVEL), \
(::qpid::sys::CATEGORY) }
