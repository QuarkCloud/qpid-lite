#ifndef QMF2_HANDLE_H
#define QMF2_HANDLE_H
...
template <class T> class Handle {
  public:
    ...
    QMF2_INLINE_EXTERN bool isValid() const { return impl; }

    /**@return true if handle is null. It is an error to call any function on a null handle. */
    QMF2_INLINE_EXTERN bool isNull() const { return !impl; }

    /** Conversion to bool supports idiom if (handle) { handle->... } */
    QMF2_INLINE_EXTERN operator bool() const { return impl; }

    /** Operator ! supports idiom if (!handle) { do_if_handle_is_null(); } */
    QMF2_INLINE_EXTERN bool operator !() const { return !impl; }

    void swap(Handle<T>& h) { T* t = h.impl; h.impl = impl; impl = t; }

  protected:
    typedef T Impl;
    QMF2_INLINE_EXTERN Handle() :impl() {}

    // Not implemented,subclasses must implement.
    QMF2_EXTERN Handle(const Handle&);
    QMF2_EXTERN Handle& operator=(const Handle&);
