#ifndef QMF2_MANAGEMENT_OBJECT_H
#define QMF2_MANAGEMENT_OBJECT_H 1
...
public:
    ...
    QMF2_EXTERN void setBanks(uint32_t broker, uint32_t bank);
...
public:
    QMF2_INLINE_EXTERN ObjectId() : agent(0), first(0), second(0), agentEpoch(0) {}
	QMF2_INLINE_EXTERN ObjectId(const qpid::types::Variant& map) :
    agent(0), first(0), second(0), agentEpoch(0) { mapDecode(map.asMap()); }
    QMF2_EXTERN ObjectId(uint8_t flags, uint16_t seq, uint32_t broker);
	QMF2_EXTERN ObjectId(AgentAttachment* _agent, uint8_t flags, uint16_t seq);
	QMF2_EXTERN ObjectId(std::istream&);
	QMF2_EXTERN ObjectId(const std::string&);
	QMF2_INLINE_EXTERN ObjectId(const std::string& agentAddress, const std::string& key,
                                uint64_t epoch=0) : agent(0), first(0), second(0),
      agentEpoch(epoch), v2Key(key), agentName(agentAddress) {}

    // Deprecated:
	QMF2_EXTERN ObjectId(uint8_t flags, uint16_t seq, uint32_t broker, uint64_t object);
	QMF2_EXTERN bool operator==(const ObjectId &other) const;
	QMF2_EXTERN bool operator<(const ObjectId &other) const;
	QMF2_EXTERN void mapEncode(::qpid::types::Variant::Map& map) const;
	QMF2_EXTERN void mapDecode(const ::qpid::types::Variant::Map& map);
	QMF2_EXTERN operator ::qpid::types::Variant::Map() const;
	QMF2_INLINE_EXTERN uint32_t encodedSize() const { return 16; };
	QMF2_EXTERN void encode(std::string& buffer) const;
	QMF2_EXTERN void decode(const std::string& buffer);
	QMF2_EXTERN bool equalV1(const ObjectId &other) const;
	QMF2_INLINE_EXTERN void setV2Key(const std::string& _key) { v2Key = _key; }
	QMF2_EXTERN void setV2Key(const ManagementObject& object);
	QMF2_INLINE_EXTERN void setAgentName(const std::string& _name) { agentName = _name; }
	QMF2_INLINE_EXTERN const std::string& getAgentName() const { return agentName; }
	QMF2_INLINE_EXTERN const std::string& getV2Key() const { return v2Key; }
    friend QMF2_EXTERN std::ostream& operator<<(std::ostream&, const ObjectId&);
};
...
class QMF2_CLASS_EXTERN ManagementObject : public ManagementItem
{
protected:
    // Thread safe wrapper for Manageable* with atomic calls and destroy().
    class QMF2_CLASS_EXTERN ManageablePtr {
        Manageable* ptr;
        mutable Mutex lock;
      ...
      public:
		...
		QMF2_EXTERN uint32_t ManagementMethod(uint32_t methodId, Args& args, std::string& text);
		QMF2_EXTERN bool AuthorizeMethod(uint32_t methodId, Args& args, const std::string& userId);
        void reset();
    };
	...
	QMF2_EXTERN int  getThreadIndex();
	QMF2_EXTERN void writeTimestamps(std::string& buf) const;
	QMF2_EXTERN void readTimestamps(const std::string& buf);
	QMF2_EXTERN uint32_t writeTimestampsSize() const;

  public:
    typedef boost::shared_ptr<ManagementObject> shared_ptr;

	QMF2_EXTERN static const uint8_t MD5_LEN = 16;
	QMF2_EXTERN static int maxThreads;
    //typedef void (*writeSchemaCall_t) (qpid::framing::Buffer&);
    typedef void (*writeSchemaCall_t) (std::string&);

	QMF2_EXTERN ManagementObject(Manageable* _core);
    ...
    // for this object.
    virtual void mapEncodeValues(qpid::types::Variant::Map& map,
                                 bool includeProperties,
                                 bool includeStatistics) = 0;
    virtual void mapDecodeValues(const qpid::types::Variant::Map& map) = 0;
    virtual void doMethod(std::string&           methodName,
                          const qpid::types::Variant::Map& inMap,
		qpid::types::Variant::Map& outMap,
                          const std::string& userId) = 0;
	QMF2_EXTERN void writeTimestamps(qpid::types::Variant::Map& map) const;
	QMF2_EXTERN void readTimestamps(const qpid::types::Variant::Map& buf);
	...
	QMF2_EXTERN virtual void setReference(ObjectId objectId);
	...
	QMF2_EXTERN void setUpdateTime();
	QMF2_EXTERN void resourceDestroy();
...
#endif  /*!QMF2_MANAGEMENT_OBJECT_H*/
