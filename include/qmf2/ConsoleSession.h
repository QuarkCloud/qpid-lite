#ifndef QMF2_CONSOLE_SESSION_H
#define QMF2_CONSOLE_SESSION_H
/*
 *
...
namespace qmf2 {
    ...
    class QMF2_CLASS_EXTERN ConsoleSession : public qmf2::Handle<ConsoleSessionImpl> {
    public:
        QMF2_EXTERN ConsoleSession(ConsoleSessionImpl* impl = 0);
        QMF2_EXTERN ConsoleSession(const ConsoleSession&);
        QMF2_EXTERN ConsoleSession& operator=(const ConsoleSession&);
        QMF2_EXTERN ~ConsoleSession();
         ...
         */
        QMF2_EXTERN ConsoleSession(qpid::messaging::Connection& conn, const std::string& options="");
        ...
        QMF2_EXTERN void setAgentFilter(const std::string& filter);
         ...
         */
        QMF2_EXTERN bool nextEvent(ConsoleEvent& outEvent, qpid::messaging::Duration timeout=qpid::messaging::Duration::FOREVER);
         ...
         */
        QMF2_EXTERN uint32_t getAgentCount() const;
        QMF2_EXTERN Agent getAgent(uint32_t agentIndex) const;

        /**
         * Get the agent for the connected broker (i.e. the agent embedded in the broker to which we have a connection).
         */
        QMF2_EXTERN Agent getConnectedBrokerAgent() const;
         ...
         */
        QMF2_EXTERN Subscription subscribe(const Query& query,       const std::string& agentFilter = "", const std::string& options = "");
        QMF2_EXTERN Subscription subscribe(const std::string& query, const std::string& agentFilter = "", const std::string& options = "");

#ifndef SWIG
    private:
        friend class qmf2::PrivateImplRef<ConsoleSession>;
