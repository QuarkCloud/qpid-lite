#ifndef QMF2_AGENT_SESSION_H
#define QMF2_AGENT_SESSION_H
/*
 *
...
namespace qmf2 {
    ...
    class QMF2_CLASS_EXTERN AgentSession : public qmf2::Handle<AgentSessionImpl> {
    public:
        QMF2_EXTERN AgentSession(AgentSessionImpl* impl = 0);
        QMF2_EXTERN AgentSession(const AgentSession&);
        QMF2_EXTERN AgentSession& operator=(const AgentSession&);
        QMF2_EXTERN ~AgentSession();
         ...
         */
        QMF2_EXTERN AgentSession(qpid::messaging::Connection& conn, const std::string& options="");
         ...
         */
        QMF2_EXTERN void setDomain(const std::string& domain);
         ...
         */
        QMF2_EXTERN void setVendor(const std::string& vendor);
        QMF2_EXTERN void setProduct(const std::string& product);
        QMF2_EXTERN void setInstance(const std::string& instance);
         ...
         * This must be called prior to opening the agent session.
         */
        QMF2_EXTERN void setAttribute(const std::string& key, const qpid::types::Variant& value);
         ...
         */
        QMF2_EXTERN bool nextEvent(AgentEvent& outEvent, qpid::messaging::Duration timeout=qpid::messaging::Duration::FOREVER);

        /**
         * Return the number of events pending for nextEvent.  This method will never block.
         */
        QMF2_EXTERN int pendingEvents() const;

        /**
         * Register a schema to be exposed by this agent.
         */
        QMF2_EXTERN void registerSchema(Schema& schema);
         ...
         */
        QMF2_EXTERN DataAddr addData(Data& data, const std::string& name="", bool persistent=false);

        /**
         * Delete data from internal agent management.
         */
        QMF2_EXTERN void delData(const DataAddr& dataAddr);
         ...
         */
        QMF2_EXTERN void authAccept(AgentEvent& event);
        QMF2_EXTERN void authReject(AgentEvent& event, const std::string& diag="");
        QMF2_EXTERN void raiseException(AgentEvent& event, const std::string& errorText);
        QMF2_EXTERN void raiseException(AgentEvent& event, const Data& errorData);
        QMF2_EXTERN void response(AgentEvent& event, const Data& responseData);
        QMF2_EXTERN void complete(AgentEvent& event);
        QMF2_EXTERN void methodSuccess(AgentEvent& event);
         ...
         *        to SEV_NOTICE.
         */
        QMF2_EXTERN void raiseEvent(const Data& data);
        QMF2_EXTERN void raiseEvent(const Data& data, int severity);

#ifndef SWIG
    private:
        friend class qmf2::PrivateImplRef<AgentSession>;
