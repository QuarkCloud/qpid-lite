...
namespace qpid {
	namespace amqp1_0 {
		...
		SaslReader::SaslReader(const std::string& i) : id(i), buffer(2 * 512/*AMQP 1.0's MAX_MIN_FRAME_SIZE - is this enough though?*/), encoder(&buffer[0], buffer.size()) {}
		SaslReader::~SaslReader() {}

		void* SaslReader::startFrame()
		{
		...
		std::size_t SaslReader::write(char* data, size_t size)
		{
		...
		std::size_t SaslReader::readProtocolHeader(const char* buffer, std::size_t size)
		{
		...
		std::size_t SaslReader::writeProtocolHeader(char* buffer, std::size_t size)
		{
			framing::ProtocolInitiation pi(qpid::framing::ProtocolVersion(1, 0, qpid::framing::ProtocolVersion::SASL));
			if (size >= pi.encodedSize()) {
