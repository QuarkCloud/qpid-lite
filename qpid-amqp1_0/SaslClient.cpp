...
namespace qpid {
	namespace amqp1_0 {
		...
		SaslClient::SaslClient(const std::string& id) : SaslReader(id) {}
		...
		namespace {
			...
			class SaslMechanismsReader : public amqp::Reader
			{
			public:
				SaslMechanismsReader(SaslClient& c) : client(c), expected(0) {}
				void onSymbol(const amqp::CharSequence& mechanism, const amqp::Descriptor*)
				{
					if (expected) {
				...
				bool onStartArray(uint32_t count, const amqp::CharSequence&, const amqp::Constructor&, const amqp::Descriptor*)
				{
					expected = count;
					return true;
				}
				void onEndArray(uint32_t, const amqp::Descriptor*)
				{
			...
			class SaslChallengeReader : public amqp::Reader
			{
			public:
				SaslChallengeReader(SaslClient& c) : client(c) {}
				void onNull(const amqp::Descriptor*) { client.challenge(); }
				void onBinary(const amqp::CharSequence& c, const amqp::Descriptor*) { client.challenge(c.str()); }
			private:
				SaslClient& client;
			};
			class SaslOutcomeReader : public amqp::Reader
			{
			public:
				SaslOutcomeReader(SaslClient& c, bool e) : client(c), expectExtraData(e) {}
				void onUByte(uint8_t c, const amqp::Descriptor*)
				{
					if (expectExtraData) code = c;
					else client.outcome(c);
				}
				void onBinary(const amqp::CharSequence& extra, const amqp::Descriptor*) { client.outcome(code, extra.str()); }
				void onNull(const amqp::Descriptor*) { client.outcome(code); }
			private:
				SaslClient& client;
		...
		bool SaslClient::onStartList(uint32_t count, const amqp::CharSequence& arguments, 
