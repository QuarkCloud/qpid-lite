...
namespace qpid {
	namespace amqp1_0 {
		...
		Sasl::Sasl(const std::string& id, ConnectionContext& c, const std::string& hostname_)
			: qpid::amqp1_0::SaslClient(id), context(c),
			sasl(qpid::sys::SaslFactory::getInstance().create(c.username, c.password, c.service, hostname_, 
				c.minSsf, c.maxSsf, false)),
		...
		void Sasl::mechanisms(const std::string& offered)
		{
			...
			if (context.mechanism.size()) {
				std::vector<std::string> allowed = sys::split(context.mechanism, " ");
				std::vector<std::string> supported = sys::split(offered, " ");
		...
		bool Sasl::authenticated()
		{
			...
			case FAILED: throw qpid::amqp::AuthenticationFailure(error.size() ? error : DEFAULT_ERROR);
