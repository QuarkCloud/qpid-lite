...
namespace qpid {
	namespace amqp1_0 {
		...
		SenderOptions::SenderOptions(bool setToOnSend_, uint32_t maxDeliveryAttempts_, bool raiseRejected_,
			const qpid::sys::NanoDuration& d)
			: setToOnSend(setToOnSend_), maxDeliveryAttempts(maxDeliveryAttempts_), 
			raiseRejected(raiseRejected_), redeliveryTimeout(d) {}


		//TODO: proper conversion to wide string for address
		SenderContext::SenderContext(pn_session_t* session, const std::string& n,
			const qpid::amqp::Address& a,
		...
		{
			error.raise();
			if (c < deliveries.size()) throw qpid::amqp::SenderError("Desired capacity is less than unsettled message count!");
			capacity = c;
		}
		...
		bool SenderContext::send(const qpid::amqp::Message& message, SenderContext::Delivery** out)
		{
			error.raise();
			...
			if (processUnsettled(false) < capacity && pn_link_credit(sender)) {
				...
				if (unreliable) {
					...
					delivery.encode(amqp::MessageImplAccess::get(message), address, options.setToOnSend);
		...
		namespace {
			...
			{
			public:
				HeaderAdapter(const qpid::amqp::MessageImpl& impl) : msg(impl), headers(msg.getHeaders()) {}
			...
			private:
				const qpid::amqp::MessageImpl& msg;
			...
			{
			public:
				PropertiesAdapter(const qpid::amqp::MessageImpl& impl, const std::string& s, const std::string& t) : msg(impl), headers(msg.getHeaders()), subject(s), to(t) {}
			...
			bool changedSubject(const qpid::amqp::MessageImpl& msg, const qpid::amqp::Address& address)
			{
		...
		namespace {
			qpid::sys::AbsTime until(const qpid::sys::NanoDuration& d)
			{
				return d ? qpid::sys::AbsTime(qpid::sys::now(), d) : qpid::sys::FAR_FUTURE;
			}
		}

		SenderContext::Delivery::Delivery(int32_t i, const uint32_t max_attempts_, const qpid::sys::NanoDuration& max_time) :
		...
		void SenderContext::Delivery::encode(const qpid::amqp::MessageImpl& msg, const qpid::amqp::Address& address, bool setToField)
		{
			try {
				boost::shared_ptr<const amqp::EncodedMessage> original = msg.getEncoded();
		...
		bool SenderContext::Delivery::delivered()
		{
			if (settled) {
				...
				if (delivery_refused()) {
					throw amqp::MessageRejected(Msg() << "delivery " << id << " refused: " << getStatus());
				}
				else if (not_delivered()) {
					if (max_attempts && (attempts >= max_attempts)) {
						throw amqp::MessageRejected(Msg() << "delivery " << id << " cannot be delivered after " << attempts << " attempts");
					}
					else if (qpid::sys::now() > retry_until) {
						throw amqp::MessageRejected(Msg() << "delivery " << id << " cannot be delivered, timed out after " << attempts << " attempts");
					}
					else {
		...
		void SenderContext::verify()
		{
			...
			if (!helper.isNameNull() && !pn_terminus_get_address(target)) {
				...
				msg += getTarget();
				QPID_LOG(debug, msg);
				throw qpid::amqp::NotFound(msg);
			}
			else if (amqp::Address::isTemporary(address)) {
		...
		amqp::Address SenderContext::getAddress() const
		{
			return address;
		}
		...
		{
			if (!error && sender) {
				error = new amqp::LinkError("sender no longer valid");
				pn_link_free(sender);
				sender = 0;
