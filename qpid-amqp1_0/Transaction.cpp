...
namespace qpid {
	namespace amqp1_0 {
		...
		Transaction::Transaction(pn_session_t* session) :
			SenderContext(session, TX_COORDINATOR, amqp::Address(ADDRESS), SenderOptions(false, true, true)), committing(false)
		{}
		...
		void Transaction::declare(SendFunction send, const SessionPtr& session) {
			...
			send(session, shared_from_this(), amqp::Message(declare), true, &delivery);
			setId(*delivery);
		}
		...
		void Transaction::discharge(SendFunction send, const SessionPtr& session, bool fail) {
			...
			try {
				...
				SenderContext::Delivery* delivery = 0;
				send(session, shared_from_this(), amqp::Message(discharge), true, &delivery);
				if (!delivery->accepted())
					throw amqp::TransactionAborted(delivery->error());
				committing = false;
			}
			catch (const amqp::TransactionError&) {
				throw;
			}
			catch (const Exception& e) {
				committing = false;
				throw amqp::TransactionAborted(e.what());
			}
		}
		...
		void Transaction::setId(const SenderContext::Delivery& delivery)
		{
			...
			throw amqp::TransactionError("No transaction ID returned by remote coordinator.");
		}
