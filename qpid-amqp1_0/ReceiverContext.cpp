...
namespace qpid {
	namespace amqp1_0 {
		...
		ReceiverContext::ReceiverContext(pn_session_t* session, const std::string& n, const qpid::amqp::Address& a)
			: name(n),
			address(a),
			helper(address),
		...
		amqp::Address ReceiverContext::getAddress() const
		{
			return address;
		}
		...
		{
			if (!error && receiver) {
				error = new amqp::LinkError("receiver no longer valid");
