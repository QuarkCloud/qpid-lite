...
namespace qpid {
	namespace amqp1_0 {
		...
		namespace {
			...
			public:
				Verifier();
				void verify(const amqp::Address& address) const;
			private:
				Variant::Map defined;
			...
			bool getAddressOption(const amqp::Address& address, const std::string& name, std::string& variable)
			{
				return getOption(address.getOptions(), name, variable);
			}

			bool getAddressOption(const amqp::Address& address, const std::string& name, Variant::Map& variable)
			{
				return getOption(address.getOptions(), name, variable);
			}
		...
		AddressHelper::AddressHelper(const amqp::Address& address) :
			isTemporary(amqp::Address::isTemporary(address)),
			...
			browse(false)
		{
			...
			if (getAddressOption(address, MODE, mode)) {
				if (mode == BROWSE) {
					...
					throw qpid::amqp::AddressError("Invalid value for mode; must be 'browse' or 'consume'.");
				}
			}

			if (!deletePolicy.empty()) {
				throw qpid::amqp::AddressError("Delete policies not supported over AMQP 1.0.");
			}
			if (node.find(X_BINDINGS) != node.end()) {
				throw qpid::amqp::AddressError("Node scoped x-bindings element not supported over AMQP 1.0.");
			}
			if (link.find(X_BINDINGS) != link.end()) {
				throw qpid::amqp::AddressError("Link scoped x-bindings element not supported over AMQP 1.0.");
			}
			if (link.find(X_SUBSCRIBE) != link.end()) {
				throw qpid::amqp::AddressError("Link scoped x-subscribe element not supported over AMQP 1.0.");
			}
			if (link.find(X_DECLARE) != link.end()) {
				throw qpid::amqp::AddressError("Link scoped x-declare element not supported over AMQP 1.0.");
			}
			...
			if (filter != link.end()) {
				if (filter->second.getType() == qpid::types::VAR_MAP) {
					...
					throw qpid::amqp::AddressError("Filter must be a map or a list of maps, each containing name, descriptor and value.");
				}
			}
		}
		...
		void AddressHelper::addFilter(const qpid::types::Variant::Map& f)
		{
			...
			if (name == f.end()) {
				throw qpid::amqp::AddressError("Filter entry must specify name");
			}
			if (descriptor == f.end()) {
				throw qpid::amqp::AddressError("Filter entry must specify descriptor");
			}
			if (value == f.end()) {
				throw qpid::amqp::AddressError("Filter entry must specify value");
			}
			try {
		...
		void AddressHelper::checkAssertion(pn_terminus_t* terminus, CheckMode mode)
		{
			if (assertEnabled(mode)) {
				...
				if (desired.size()) {
					...
					throw qpid::amqp::AssertionFailed(missing.str());
				}
				...
				if (!first) throw qpid::amqp::AssertionFailed(missing.str());
		...
		std::string AddressHelper::getLinkName(const amqp::Address& address)
		{
		...
		void Verifier::verify(const amqp::Address& address) const
		{
		...
		void Verifier::verify(const Variant::Map& allowed, const Variant::Map& actual) const
		{
			for (Variant::Map::const_iterator i = actual.begin(); i != actual.end(); ++i) {
				...
				if (option == allowed.end()) {
					throw amqp::AddressError((boost::format("Unrecognised option: %1%") % i->first).str());
				}
