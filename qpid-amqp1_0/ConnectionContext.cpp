...
#include "qpid/amqp1_0/Transaction.h"
#include "qpid/amqp1_0/util.h"
...
namespace amqp1_0 {
	...
	namespace {
		...
		public:
			ConnectionTickerTask(const qpid::sys::NanoDuration& interval, qpid::sys::Timer& t, ConnectionContext& c) :
...
ConnectionContext::ConnectionContext(const std::string& url, const qpid::types::Variant::Map& o)
    : qpid::amqp::ConnectionOptions(o),
      fullUrl(url, protocol.empty() ? qpid::sys::Address::TCP : protocol),
      engine(pn_transport()),
...
void ConnectionContext::close()
{
    ...
    if (!(pn_connection_state(connection) & PN_LOCAL_CLOSED)) {
        for (SessionMap::iterator i = sessions.begin(); i != sessions.end(); ++i) {
            try {
                syncLH(i->second, l);
            } catch (const amqp::MessageRejected& e) {
            ...
            } catch (const amqp::SessionClosed& ) {
                continue;
            } catch (const amqp::TransactionAborted& ) {
                continue;
            }
...
bool ConnectionContext::fetch(boost::shared_ptr<SessionContext> ssn, boost::shared_ptr<ReceiverContext> lnk, 
	qpid::amqp::Message& message, qpid::sys::MilliDuration timeout)
{
    ...
    if (get(ssn, lnk, message, timeout)) {
        ...
        if (get(ssn, lnk, message, qpid::sys::MilliDuration::IMMEDIATE)) {
            return true;
        } else {
            return false;
        }
...
qpid::sys::AbsTime convert(qpid::sys::MilliDuration timeout)
{
...
bool ConnectionContext::get(boost::shared_ptr<SessionContext> ssn, boost::shared_ptr<ReceiverContext> lnk, 
	qpid::amqp::Message& message, qpid::sys::MilliDuration timeout)
{
    ...
    while (true) {
        ...
        if (current && !pn_delivery_partial(current)) {
            qpid::amqp::MessageImpl& impl = amqp::MessageImplAccess::get(message);
            boost::shared_ptr<amqp::EncodedMessage> encoded(new amqp::EncodedMessage(pn_delivery_pending(current)));
            encoded->setNestAnnotationsOption(nestAnnotations);
            ssize_t read = pn_link_recv(lnk->receiver, encoded->getData(), encoded->getSize());
            if (read < 0) throw qpid::amqp::MessagingException("Failed to read message");
...
boost::shared_ptr<ReceiverContext> ConnectionContext::nextReceiver(boost::shared_ptr<SessionContext> ssn, 
...
void ConnectionContext::acknowledge(boost::shared_ptr<SessionContext> ssn, qpid::amqp::Message* message, bool cumulative) {
    sys::Monitor::ScopedLock l(lock);
    acknowledgeLH(ssn, message, cumulative, l);
}

void ConnectionContext::acknowledgeLH(boost::shared_ptr<SessionContext> ssn, qpid::amqp::Message* message,
	bool cumulative, sys::Monitor::ScopedLock&)
{
    checkClosed(ssn);
    if (message) {
        ssn->acknowledge(amqp::MessageImplAccess::get(*message).getInternalId(), cumulative);
    } else {
        ssn->acknowledge();
    }
    wakeupDriver();
...
void ConnectionContext::nack(boost::shared_ptr<SessionContext> ssn, qpid::amqp::Message& message, bool reject)
{
    sys::Monitor::ScopedLock l(lock);
    checkClosed(ssn);
    ssn->nack(amqp::MessageImplAccess::get(message).getInternalId(), reject);
    wakeupDriver();
}
...
qpid::amqp::Address ConnectionContext::passthrough(const qpid::amqp::Address& in)
{
    qpid::amqp::Address out;
    out.setName(in.str());
...
boost::shared_ptr<SenderContext> ConnectionContext::createSender(boost::shared_ptr<SessionContext> session, 
	const qpid::amqp::Address& address)
{
    ...
    boost::shared_ptr<SenderContext> sender =
        session->createSender(usePassthrough() ? passthrough(address) : address,
                              SenderOptions(setToOnSend, maxDeliveryAttempts, raiseRejected, (int64_t)(redeliveryTimeout * qpid::sys::TIME_SEC)));
    try {
...
boost::shared_ptr<ReceiverContext> ConnectionContext::createReceiver(boost::shared_ptr<SessionContext> session, 
	const qpid::amqp::Address& address)
{
    sys::Monitor::ScopedLock l(lock);
...
void ConnectionContext::send(
    ...
    const qpid::amqp::Message& message,
    bool sync,
...
bool ConnectionContext::check() {
    if (checkDisconnected()) {
        if (ConnectionOptions::reconnect) {
            ...
            throw qpid::amqp::TransportFailure("Disconnected (reconnect disabled)");
        }
        return true;
    }
    return false;
...
bool ConnectionContext::checkDisconnected() {
    if (state == DISCONNECTED) {
        ...
        if ((pn_connection_state(connection) & REQUIRES_CLOSE) == REQUIRES_CLOSE) {
            ...
            pn_connection_close(connection);
            throw qpid::amqp::ConnectionError(text);
        }
    }
...
void ConnectionContext::checkClosed(boost::shared_ptr<SessionContext> ssn)
{
    ...
    if ((pn_session_state(ssn->session) & REQUIRES_CLOSE) == REQUIRES_CLOSE) {
        ...
        pn_session_close(ssn->session);
        throw qpid::amqp::SessionError(text);
...
void ConnectionContext::checkClosed(boost::shared_ptr<SessionContext> ssn, pn_link_t* lnk)
{
    ...
    if ((pn_link_state(lnk) & REQUIRES_CLOSE) == REQUIRES_CLOSE) {
        ...
        if (name == qpid::amqp::error_conditions::NOT_FOUND) {
            throw qpid::amqp::NotFound(text);
        } else if (name == qpid::amqp::error_conditions::UNAUTHORIZED_ACCESS) {
            throw qpid::amqp::UnauthorizedAccess(text);
        } else {
            throw qpid::amqp::LinkError(text);
        }
    } else if ((pn_link_state(lnk) & IS_CLOSED) == IS_CLOSED) {
        throw qpid::amqp::LinkError("Link is not attached");
    }
}
...
boost::shared_ptr<SessionContext> ConnectionContext::newSession(bool transactional, const std::string& n)
{
    ...
    std::string name = n.empty() ? qpid::framing::Uuid(true).str() : n;
    {
        ...
        if (i == sessions.end()) {
            ...
            throw qpid::amqp::KeyError(std::string("Session already exists: ") + name);
        }
...
boost::shared_ptr<SessionContext> ConnectionContext::getSession(const std::string& name) const
{
    SessionMap::const_iterator i = sessions.find(name);
    if (i == sessions.end()) {
        throw qpid::amqp::KeyError(std::string("No such session") + name);
    } else {
        return i->second;
    }
}
...
std::size_t ConnectionContext::decodePlain(const char* buffer, std::size_t size)
{
    ...
    if (n > 0 || n == PN_EOS) {
        ...
        int64_t now = qpid::sys::NanoDuration(qpid::sys::ZERO, qpid::sys::AbsTime::now());
...
sys::ProtocolVersion ConnectionContext::getVersion() const
{
...
const qpid::amqp::ConnectionOptions* ConnectionContext::getOptions()
{
    return this;
}
...
std::size_t ConnectionContext::decode(const char* buffer, std::size_t size)
{
    ...
    } catch (const amqp::AuthenticationFailure&) {
        transport->close();
    }
    return decoded;
}
...
{
    sys::Monitor::ScopedLock l(lock);
    if (state != DISCONNECTED) throw qpid::amqp::ConnectionError("Connection was already opened!");
    if (!driver) driver = amqp::DriverImpl::getDefault();
...
bool expired(const sys::AbsTime& start, double timeout)
{
    ...
    qpid::sys::NanoDuration used(start, qpid::sys::now());
    qpid::sys::NanoDuration allowed((int64_t)(timeout*qpid::sys::TIME_SEC));
    return allowed < used;
}
const std::string COLON(":");
}

void throwConnectFail(const sys::Url& url, const std::string& msg) {
    throw qpid::sys::TransportFailure(
        sys::Msg() << "Connect failed to " << url << ": " << msg);
}
...
void ConnectionContext::reconnect(const sys::Url& url) {
...
void ConnectionContext::reconnect(const std::string& url) { reconnect(sys::Url(url)); }
...
bool ConnectionContext::tryConnectUrl(const sys::Url& url)
{
    if (url.getUser().size()) username = url.getUser();
    if (url.getPass().size()) password = url.getPass();

    for (sys::Url::const_iterator i = url.begin(); i != url.end(); ++i) {
...
bool ConnectionContext::tryOpenAddr(const qpid::sys::Address& addr) {
    currentUrl = sys::Url(addr);
    if (sasl.get()) {
        wakeupDriver();
    ...
    if (!(pn_connection_state(connection) & PN_REMOTE_ACTIVE)) {
        throw qpid::amqp::ConnectionError("Failed to open connection");
    }
    ...
    if (shortest) {
        ...
        qpid::sys::NanoDuration d(shortest * qpid::sys::TIME_MSEC);
...
bool ConnectionContext::tryConnectAddr(const qpid::sys::Address& address)
{
    ...
    transport = boost::shared_ptr<sys::Transport>();
    return false;
}
...
void ConnectionContext::startTxSession(boost::shared_ptr<SessionContext> session) {
    try {
        ...
        throw amqp::TransactionError(sys::Msg() << "Cannot start transaction: " << e.what());
    }
}
...
void ConnectionContext::discharge(boost::shared_ptr<SessionContext> session, bool fail) {
    {
        ...
        if (!session->transaction)
            throw amqp::TransactionError("No Transaction");
