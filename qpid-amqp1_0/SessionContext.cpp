...
namespace qpid {
	namespace amqp1_0 {
		...
		boost::shared_ptr<SenderContext> SessionContext::createSender(const qpid::amqp::Address& address, const SenderOptions& options)
		{
			error.raise();
			std::string name = AddressHelper::getLinkName(address);
			if (senders.find(name) != senders.end())
				throw amqp::LinkError("Link name must be unique within the scope of the connection");
		...
		boost::shared_ptr<ReceiverContext> SessionContext::createReceiver(const qpid::amqp::Address& address)
		{
			error.raise();
			std::string name = AddressHelper::getLinkName(address);
			if (receivers.find(name) != receivers.end()) throw amqp::LinkError("Link name must be unique within the scope of the connection");
		...
		boost::shared_ptr<SenderContext> SessionContext::getSender(const std::string& name) const
		{
			...
			if (i == senders.end()) {
				throw qpid::amqp::KeyError(std::string("No such sender") + name);
			}
			else {
				return i->second;
			}
		...
		boost::shared_ptr<ReceiverContext> SessionContext::getReceiver(const std::string& name) const
		{
			...
			if (i == receivers.end()) {
				throw qpid::amqp::KeyError(std::string("No such receiver") + name);
			}
			else {
				return i->second;
			}
		...
		bool SessionContext::settled()
		{
			...
			for (SenderMap::iterator i = senders.begin(); i != senders.end(); ++i) {
				...
				catch (const amqp::MessageRejected&) {
					throw;
				}
		...
		void SessionContext::reset(pn_connection_t* connection)
		{
			...
			if (transaction) {
				if (transaction->isCommitting())
					error = new amqp::TransactionUnknown("Transaction outcome unknown: transport failure");
				else
					error = new amqp::TransactionAborted("Transaction aborted: transport failure");
				resetSession(0);
				senders.clear();
				receivers.clear();
				transaction.reset();
