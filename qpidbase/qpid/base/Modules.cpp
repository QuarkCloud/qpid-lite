...
namespace qpid {
	namespace base {
		...
		ModuleOptions::ModuleOptions(const std::string& defaultModuleDir)
			: qpid::sys::Options("Module options"), loadDir(defaultModuleDir), noLoad(false)
		{
			addOptions()
				("module-dir", sys::optValue(loadDir, "DIR"), "Load all shareable modules in this directory")
				("load-module", sys::optValue(load, "FILE"), "Specifies additional module(s) to be loaded")
				("no-module-dir", sys::optValue(noLoad), "Don't load modules from module directory");
		}
		...
		void loadModuleDir(std::string dirname, bool isDefault)
		{
			...
			try
			{
				...
				throw sys::Exception("Invalid value for module-dir: " + e.getMessage());
			}
			if (!exists) {
				if (isDefault) return;
				throw sys::Exception("Directory not found: " + dirname);
			}
