...
#include "qpid/base/amqp0_10/CodecsInternal.h"
...
namespace qpid {
	namespace base {
		namespace amqp0_10 {
			...
			Variant::Map::value_type toVariantMapEntry(const qpid::framing::FieldTable::value_type& in);

			template <class T, class U, class F> void translate(boost::shared_ptr<qpid::framing::FieldValue> in, U& u, F f)
			{
				T t;
			...
			uint32_t encodedSize(const Variant& value)
			{
				...
				default:
					throw qpid::sys::Exception("Couldn't encode Variant: Illegal type code");
				}
			}
			...
			void encode(const std::string& value, const std::string& encoding, qpid::framing::Buffer& buffer)
			{
				...
				if (size > std::numeric_limits<uint16_t>::max()) {
					if (encoding == utf8 || encoding == utf16 || encoding == iso885915) {
						throw qpid::sys::Exception(QPID_MSG("Could not encode " << encoding << " character string - too long (" << size << " bytes)"));
					}
					else {
			...
			{
				uint32_t len = qpid::base::amqp0_10::encodedSize(value);
				std::vector<char> space(len);
				qpid::framing::Buffer buff(&space[0], len);

				qpid::base::amqp0_10::encode(value, len, buff);
			...
			{
				return qpid::base::amqp0_10::encodedSize(value);
			}
			...
			namespace {
				...
				boost::shared_ptr<FieldValue> convertString(const std::string& value, const std::string& encoding)
				{
					...
					if (encoding.empty() || encoding == amqp0_10_binary || encoding == binary) {
						...
						throw qpid::sys::Exception(QPID_MSG("Could not encode utf8 character string - too long (" << value.size() << " bytes)"));
					}
					else if (encoding == utf16) {
						if (!large)
							return boost::shared_ptr<FieldValue>(new Var16Value(value, 0x96));
						throw qpid::sys::Exception(QPID_MSG("Could not encode utf16 character string - too long (" << value.size() << " bytes)"));
					}
					else if (encoding == iso885915) {
						if (!large)
							return boost::shared_ptr<FieldValue>(new Var16Value(value, 0x94));
						throw qpid::sys::Exception(QPID_MSG("Could not encode iso-8859-15 character string - too long (" << value.size() << " bytes)"));
					}
					else {
