...
#include "qpid/base/SaslServer.h"
#include "qpid/base/NullSaslClient.h"
#include "qpid/base/NullSaslServer.h"
#include <map>
#include <string.h>
	...
	namespace base {
		...
		bool CyrusSasl::start(const std::string& mechanisms, std::string& response, const SecuritySettings* externalSettings)
		{
			...
			if (result == SASL_NOMECH) {
				if (mechanisms.size()) {
					throw qpid::sys::Exception(std::string("Can't authenticate using ") + mechanisms);
				}
				else {
					throw qpid::sys::Exception("No mutually acceptable authentication mechanism");
				}
			}
