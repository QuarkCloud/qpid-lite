 ...
 *
 */
#include "qpid/base/amqp/MessageEncoder.h"
#include "qpid/base/amqp/MapEncoder.h"
#include "qpid/base/amqp/MapSizeCalculator.h"
	...
	namespace base {
		namespace amqp {
			...
			void MessageEncoder::writeHeader(const Header& msg)
			{
				...
				if (fields) {
					void* token = startList8(&qpid::base::amqp::message::HEADER);
			...
			void MessageEncoder::writeProperties(const Properties& msg)
			{
				...
				if (fields) {
					void* token = startList32(&qpid::base::amqp::message::PROPERTIES);
			...
			void MessageEncoder::writeApplicationProperties(const ApplicationProperties& properties)
			{
				...
				size_t required = calc.getTotalSizeRequired(&qpid::base::amqp::message::APPLICATION_PROPERTIES);
				assert(required <= getSize() - getPosition());
				MapEncoder encoder(skip(required), required);
				encoder.writeMetaData(calc.getSize(), calc.getCount() * 2, &qpid::base::amqp::message::APPLICATION_PROPERTIES);
			...
			{
				writeMap(properties, &qpid::base::amqp::message::APPLICATION_PROPERTIES, large);
			}
			...
			size_t MessageEncoder::getEncodedSize(const ApplicationProperties& p)
			{
				MapSizeCalculator calc;
				p.handle(calc);
				return calc.getTotalSizeRequired(&qpid::base::amqp::message::APPLICATION_PROPERTIES);
			}
