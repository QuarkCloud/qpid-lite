...
namespace qpid {
	namespace base {
		namespace amqp {
			...
			Encoder::Overflow::Overflow() :qpid::sys::Exception("Buffer overflow in encoder!") {}
			...
			{
				end<uint8_t>((uint8_t)count, token, data + position);
			}
			...
			void Encoder::writeMap(const std::map<std::string, qpid::types::Variant>& value, const Descriptor* d, bool large)
			{
				...
				else endMap8((uint8_t)value.size() * 2, token);
			}
			...
			void Encoder::writeList(const std::list<qpid::types::Variant>& value, const Descriptor* d, bool large)
			{
				...
				else endList8((uint8_t)value.size(), token);
			}
