 ...
 *
 */
#include "qpid/base/amqp/SaslServer.h"
#include "qpid/base/amqp/Constructor.h"
#include "qpid/base/amqp/Decoder.h"
	...
	namespace base {
		namespace amqp {
			...
			void SaslServer::mechanisms(const std::string& mechanisms)
			{
				...
				void* listToken = encoder.startList32(&sasl::SASL_MECHANISMS);
				if (parts.size() > 1) {
					void* arrayToken = encoder.startArray8(Constructor(typecodes::SYMBOL8));
			...
			void SaslServer::challenge(const std::string* c)
			{
				...
				void* listToken = encoder.startList32(&sasl::SASL_CHALLENGE);
			...
			void SaslServer::completed(bool succeeded)
			{
				...
				void* listToken = encoder.startList8(&sasl::SASL_OUTCOME);
			...
			bool SaslServer::onStartList(uint32_t count, const CharSequence& arguments, const CharSequence& /*full raw data*/, const Descriptor* descriptor)
			{
				...
				else if (descriptor->match(sasl::SASL_INIT_SYMBOL, sasl::SASL_INIT_CODE)) {
				...
				else if (descriptor->match(sasl::SASL_RESPONSE_SYMBOL, sasl::SASL_RESPONSE_CODE)) {
