...
#include "qpid/base/amqp/MapBuilder.h"
...
namespace qpid {
	namespace base {
		namespace amqp {
			...
			using namespace qpid::base::amqp::typecodes;
			...
			Descriptor Decoder::readDescriptor()
			{
				...
				default:
					throw qpid::sys::Exception(QPID_MSG("Expected descriptor of type ulong or symbol; found " << (int)code));
				}
			}

			void Decoder::advance(size_t n)
			{
				if (n > available()) throw qpid::sys::Exception(QPID_MSG("Out of Bounds: requested advance of " << n << " at " << position << " but only " << available() << " available"));
				position += n;
			}
