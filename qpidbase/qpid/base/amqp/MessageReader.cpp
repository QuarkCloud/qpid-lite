 ...
 *
 */
#include "qpid/base/amqp/MessageReader.h"
#include "qpid/base/amqp/Descriptor.h"
#include "qpid/base/amqp/descriptors.h"
#include "qpid/base/amqp/typecodes.h"
		...
		namespace amqp {
			...
			bool MessageReader::onStartList(uint32_t count, const CharSequence& elements, const CharSequence& raw, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					else if (descriptor->match(message::HEADER_SYMBOL, message::HEADER_CODE)) {
						delegate = &headerReader;
						return true;
					}
					else if (descriptor->match(message::PROPERTIES_SYMBOL, message::PROPERTIES_CODE)) {
						delegate = &propertiesReader;
						return true;
					}
					else if (descriptor->match(message::AMQP_SEQUENCE_SYMBOL, message::AMQP_SEQUENCE_CODE)) {
						onAmqpSequence(raw);
						return false;
					}
					else if (descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(elements, qpid::base::amqp::typecodes::LIST_NAME, nested(descriptor));
						return false;
					}
					else {
			...
			{
				if (delegate) {
					if (descriptor && (descriptor->match(message::HEADER_SYMBOL, message::HEADER_CODE) || 
						descriptor->match(message::PROPERTIES_SYMBOL, message::PROPERTIES_CODE))) {
						delegate = 0;
					}
					else {
			...
			bool MessageReader::onStartMap(uint32_t count, const CharSequence& elements, const CharSequence& raw, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					else if (descriptor->match(message::DELIVERY_ANNOTATIONS_SYMBOL, message::DELIVERY_ANNOTATIONS_CODE)) {
						onDeliveryAnnotations(elements, raw);
						return false;
					}
					else if (descriptor->match(message::MESSAGE_ANNOTATIONS_SYMBOL, message::MESSAGE_ANNOTATIONS_CODE)) {
						onMessageAnnotations(elements, raw);
						return false;
					}
					else if (descriptor->match(message::FOOTER_SYMBOL, message::FOOTER_CODE)) {
						onFooter(elements, raw);
						return false;
					}
					else if (descriptor->match(message::APPLICATION_PROPERTIES_SYMBOL, message::APPLICATION_PROPERTIES_CODE)) {
						onApplicationProperties(elements, raw);
						return false;
					}
					else if (descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(elements, qpid::base::amqp::typecodes::MAP_NAME, nested(descriptor));
						return false;
					}
					else {
			...
			void MessageReader::onBinary(const CharSequence& bytes, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					else if (descriptor->match(message::DATA_SYMBOL, message::DATA_CODE)) {
						onData(bytes);
					}
					else if (descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(bytes, qpid::base::amqp::typecodes::BINARY_NAME, nested(descriptor));
					}
					else {
			...
			void MessageReader::onNull(const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
			...
			void MessageReader::onString(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::base::amqp::typecodes::STRING_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			void MessageReader::onSymbol(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::base::amqp::typecodes::SYMBOL_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			void MessageReader::onUuid(const CharSequence& v, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						onAmqpValue(v, qpid::base::amqp::typecodes::UUID_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			bool MessageReader::onStartArray(uint32_t count, const CharSequence& raw, const Constructor& constructor, const Descriptor* descriptor)
			{
				if (delegate) {
					...
					if (descriptor && descriptor->match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE)) {
						...
						onAmqpValue(raw, qpid::base::amqp::typecodes::ARRAY_NAME, nested(descriptor));
					}
					else {
						if (!descriptor) {
			...
			{
				if (bare.data) {
					if (descriptor.match(message::FOOTER_SYMBOL, message::FOOTER_CODE)) {
						bare.size = position - bare.data;
					}
				}
				else {
					if (descriptor.match(message::PROPERTIES_SYMBOL, message::PROPERTIES_CODE) 
						|| descriptor.match(message::APPLICATION_PROPERTIES_SYMBOL, message::APPLICATION_PROPERTIES_CODE)
						|| descriptor.match(message::AMQP_SEQUENCE_SYMBOL, message::AMQP_SEQUENCE_CODE) 
						|| descriptor.match(message::AMQP_VALUE_SYMBOL, message::AMQP_VALUE_CODE) 
						|| descriptor.match(message::DATA_SYMBOL, message::DATA_CODE)) {
						bare.data = position;
					}
				}
			}
