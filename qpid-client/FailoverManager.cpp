...
FailoverManager::FailoverManager(const amqp::ConnectionOptions& s,
...
void FailoverManager::execute(Command& c)
{
    ...
    while (!completed) {
        try {
            ...
            if (retry) {
				NanoDuration failoverTime(failed, AbsTime::now());
            ...
            session.close();
            completed = true;
        } catch(const sys::TransportFailure&) {
            retry = true;
            failed = AbsTime::now();
        }            
    }
...
Connection& FailoverManager::connect(std::vector<sys::Url> brokers)
{
...
void FailoverManager::attempt(Connection& c, amqp::ConnectionOptions s, std::vector<sys::Url> urls)
{
    ...
    if (urls.empty()) {
        ...
        for (std::vector<sys::Url>::const_iterator i = urls.begin(); i != urls.end() && !c.isOpen(); ++i) {
            for (sys::Url::const_iterator j = i->begin(); j != i->end() && !c.isOpen(); ++j) {
                const sys::Address& addr = *j;
...
void FailoverManager::attempt(Connection& c, amqp::ConnectionOptions s)
{
    try {
