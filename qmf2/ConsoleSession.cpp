...
#include "qmf2/ConsoleSessionImpl.h"
#include "qmf2/AgentImpl.h"
#include "qmf2/SchemaId.h"
#include "qmf2/SchemaImpl.h"
#include "qmf2/ConsoleEventImpl.h"
...
typedef qmf2::PrivateImplRef<ConsoleSession> PI;

ConsoleSession::ConsoleSession(ConsoleSessionImpl* impl) { PI::ctor(*this, impl); }
ConsoleSession::ConsoleSession(const ConsoleSession& s) : qmf2::Handle<ConsoleSessionImpl>() { PI::copy(*this, s); }
...
void ConsoleSessionImpl::dispatch(Message msg)
{
    ...
    if (iter != properties.end() && iter->second.asString() == protocol::HEADER_APP_ID_QMF && oiter != properties.end()) {
        ...
        if (body.size() < 8)
            return;
        qmf2::Buffer buffer(const_cast<char*>(body.c_str()), body.size());
...
void ConsoleSessionImpl::handleV1SchemaResponse(qmf2::Buffer& buffer, uint32_t, const Message&)
{
