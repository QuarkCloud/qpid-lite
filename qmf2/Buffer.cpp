...
#include "qpid/messaging/amqp0_10/Codecs.h"

using namespace std;

namespace qmf2 {

Buffer::Buffer(char* data, uint32_t size) : impl(new qpid::framing::Buffer(data, size)) {}
...
void Buffer::putMap(const qpid::types::Variant::Map& i)
{
    string encoded;
	qpid::messaging::amqp0_10::MapCodec::encode(i, encoded);
    impl->putRawData(encoded);
}

void Buffer::putList(const qpid::types::Variant::List& i)
{
    string encoded;
	qpid::messaging::amqp0_10::ListCodec::encode(i, encoded);
    impl->putRawData(encoded);
}

void Buffer::getMap(qpid::types::Variant::Map& map)
{
    string encoded;
	...
	qpid::messaging::amqp0_10::MapCodec::decode(encoded, map);
}

void Buffer::getList(qpid::types::Variant::List& list)
{
    string encoded;
    uint32_t saved = impl->getPosition();
    uint32_t length = impl->getLong();
    impl->setPosition(saved);
    impl->getRawData(encoded, length + sizeof(uint32_t));
	qpid::messaging::amqp0_10::ListCodec::decode(encoded, list);
}
