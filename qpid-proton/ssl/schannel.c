                                             ...
                                             const char *session_id, SECURITY_STATUS *status)
{
  ...
  *status = AcquireCredentialsHandle(NULL, (LPSTR)UNISP_NAME, direction, NULL,
...
static bool server_name_matches(const char *server_name, CERT_EXTENSION *alt_name_ext, PCCERT_CONTEXT cert)
{
  ...
  if (!matched) {
    ...
    DWORD len = CertGetNameString(cert, CERT_NAME_ATTR_TYPE, 0, (void *)szOID_COMMON_NAME, 0, 0);
    char *name = (char *) malloc(len);
    if (name) {
      int count = CertGetNameString(cert, CERT_NAME_ATTR_TYPE, 0, (void *)szOID_COMMON_NAME, name, len);
      if (count)
